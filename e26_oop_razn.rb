...
# методы экземпляра запускаются в контексте объекта (и имеют доступ к переменным объекта или экземпляра)
# доступ к методу класса осуществляется через класс, а к методу экземпляра — через экземпляр или объект класса
...
# ПРИМЕЧАНИЕ: делать классы только со статическимиметодами не рекомендуется — нужно использовать модули. Классы следует использовать только тогда, когда имеет смысл создавать из них экземпляры.
...
class Song
  ...
  def Song.isTooLong?(aSong) # Метод получает параметром объект
    aSong.duration > 300
  end
end
song1 = Song.new(260)
p Song.isTooLong?(song1) #=> false
song2 = Song.new(468)
p Song.isTooLong?(song2) #=> true
...
# protected methods - защищённый интерфейс — защищенный метод может вызываться только объектами определяющего класса и его подклассов. Мы не можем вызвать protected метод объекта непосредственно, но один объект может вызвать защищенный метод для себя или любого другого родственного объекта
