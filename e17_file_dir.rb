...
puts
puts '                              File создание/открытие фаила. File.new и File.open'

# Между методами File.open и File.new есть разница в том, что метод File.open может быть связан с блоком, а File.new не может.
...
file = File.new("filename.extension", "r")
  # ... process the file
file.close #=> метод .close , чтобы закрыть этот файл.
...
File.open("ruby_exemples/text/simple.txt", "r") do |file| # в file Ruby присвоит указанный фаил.
...
end # file.close тут не нужен


puts
puts '                                            Запись в фаил(с блоком)'
...
# Те же самые методы, которые мы использовали для «простого» ввода-вывода, доступны для всех файловых объектов. Итак, gets читает строку из стандартного ввода, а file.gets читает строку из файлового объекта file. Однако объекты ввода-вывода предоставляют дополнительный набор методов доступа, облегчающих нашу жизнь:

# Работа со строкой пути фаила
file ="ruby_exemples/text/simple2.txt"
filename = File.basename(file) #=> "simple2.txt"
bname = File.basename(file,".txt") #=> "simple2" # 2й аргумент удаляет расширение
extention = File.extname(file) #=> ".txt"
# Получение директории без имени файла
path_name= File.dirname(file) #=> "ruby_exemples/text"
File.dirname("/var/www/test.txt") #=> /var/www
