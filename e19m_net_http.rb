...
# GET(запрос от браузера серверу на получение данных) - запрос о том что браузер хочет получить какую-то страницу(ресурс) так же он содержит о кодировках и языках поддерживаемых браузером. На этот запрос от сервера приходит ответ в виде данных запрашиваемой страницы/картинки/ресурса(ее html/css/... код) или об ошибке и некорректности запроса(например 404 страница не найдена)
# POST(запрос от браузера серверу на отправку данных на сервер) - используется для отправки браузером данных на сервер(например ввод пароля). так же содержит служебную информацию
