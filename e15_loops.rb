puts '                                      Циклы: while, antil, loop и for'
...
puts
# Цикл while с проверкой в конце. Если модификатор while следует за оператором begin без предложений восстановления или обеспечения, код выполняется один раз перед оценкой условия. Если код нужно выполнить хотябы 1 раз, чтоб не писать его еще раз перед телом цикла
i = 0
begin
  print "#{list [i]} "
  i += 1
...
for name in nameS # Будет выбирать в диапазоне массива nameS
  a = 1 # не создает новую область для локальных переменных
...
p a #=> 1 # не создает новую область для локальных переменных
print nameS # выведет без добавленных знаков ! тк сам массив не был изменен циклом а только использован

nameS.each{|name| a = 1 } # создает новую область для локальных переменных
