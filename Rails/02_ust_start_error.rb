...
# 1. Установка Руби через RVM для *nix-систем(Unix-подобная операционная система, например Линукс и МакОС):
...
# 2. (Не пробовал) В конспектах рубишколы на гитхаб так же есть ruby через RVM и другие штуки:
...
# f .git
# d app              -  содержит отдельные папки views, models, controllers итд. Основной код приложения
...
# На никс-системах мы просто вводим в терминал bin\dev и сервер запустится(берет инструции из этого фаила bin\dev сам)
...
# 2. На Винде при старте проекта или при различных манипуляциях, например при обгновленни версии в фаилы bin/rails, bin/rake и bin/setup в первую закоментированную строку в конце может добавляться расширение .exe, его нужно удалить, тк могут возникнуть проблемы при развертывании приложения в продакшене
