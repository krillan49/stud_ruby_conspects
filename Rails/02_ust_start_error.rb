...
# https://gorails.com/setup/windows/11  - гайд как поставить Рэилс на разные ОС (и их равзные версии)
...
# > rails new blog   -  команда cоздаcт новое рейлс-приложение с поледней из версий Рейлс, среди тех что есть в нашей системе, создаст в текущей папке(Устанавливаются все необходимое для работы приложения, гемы, инициализируется гит, создаются все необходимые старотовые папки и фаилы итд), где:
# rails - бинарник рельс
# new - команда для создания нового проекта
# blog - это имя директории приложения(можно с большой буквы).
...
puts '                                     Структура фаилов Рэилс приложения'

# В результате выполнения команды создания нового приложени появляется папка с выбранным именем, она содержит следующие стартовые папки и фаилы(тут f - фаил, d - папка):
...
# d config           -  конфигурация/настройки приложения:
...
# d db               -  миграции и фаилы базы данных
# d lib              -  дополнительные Руби фаилы, обычно там пусто
# d log              -  журналы событий, например логи сервера
# d public           -  фаилы передаваемые напрямую минуя приложение (статичные), просто в ответ на запрос. Тоесть их можно вызвать написав от корня имя этого фаила. А если создать там index.html то он будет автоматически открываться при заходе на сервер. В принципе можно сюда добавить любой статический сайт(лэндинг) со всеми стилями джаваскриптом, картинками и это будет работать.
# d storage          -  все что связано с гемом актив сторэдж
# d test             -  тесты, но только для MinTest (Для RSpec создается отдельная папка)
# d tmp              -  используется самим Рэилс для временных фаилов
# d vendor           -  сейчас уже особо не используется, могут быть библиотеки которые добавим не при помощи Gemfile
# f .gitattributes
# f .gitignore       -  изначально включает фаилы БД, логи, скомпилированные ассеты, мастер кей(ключ шифрования) итд
# f .ruby-version    -  лучшк удалить
# f config.ru        -  фаил описывающий как запустить наше приложение
# f Gemfile          -  из него лучше удалить версию Руби, тк гемора будет меньше чем если оставить (хз почему ??)
# f Gemfile.lock
# f Rakefile         -  фаил для рэйк-задач(кастомные задачи написанные нами в проекте) туда же будут добавляться задачи из lib/tasks
# f README.md
...
# $ bundle config set --local without 'production' - создаст строку в папке .bundle, которая задаст, что если мы локально запустим bundle, то он будет запускаться без гемов из группы продакшен, тоесть не установит их
...
# На Линукс этот гем совсем не нужен и можно его удалить или закоментить
...
# Окружение (среда) - это базы данных, опции/параметры/настройки, веб сервер итд. Все то что будет отличаться например при разработке на локальной машине и при хостинге на сервер

# Rails-приложение по умолчанию может запускаться в 3 разных типах окружения(режимах):
# 1. development    - оптимизирует среду для удобства локальной разработки, будет работать чуть медленнее.
# 2. test           - для тестирования
# 3. production     - запускает только то, что нужно для работы приложения, работает максимально быстро

# Для каждого типа окружения по умолчапнию существует своя отдельная БД
...
# Помимо окружения по умолчанию, могут быть и другие типы окружения, например для деплоя на разнве серверы может быть разное
