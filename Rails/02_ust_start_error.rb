...
# 3. Устанавливаем Node.js (в конспектах по ЖС)

# 4. Установить пакетный менеджер Yarn, он позволяет добавлять всякие штуки для фронтэнда (в конспектах по ЖС)
...
# 1. Node.js и Yarn (в конспектах по ЖС)

# 2. Rails:
# Посмотрим доступные версии Rails:
# $ gem search '^rails$' --all
# Чтобы установить конкретную версию (тут 7.0.7):
...
# $ gem list rails              -  посмотреть гемы в имени которых есть "rails", удобно так будет посмотреть все установленные версии Рэилс



puts '                                           Создание приложения'

# > rails new blog   -  команда cоздаcт новое рейлс-приложение с поледней из версий Рейлс, среди тех что есть в нашей системе, создаст в текущей папке, где:
...
# $ rails _4.2.7_ new blog  - создать новое приложение под определенную версию Рэилс, тут 4.2.7 (?? только для *никс)
...
# > start rails s
# > bundle exec rails s    - запускает Рэилс приложение с набором именно тех гемов что есть в Gemfile

# запускает веб-сервер Puma
# Далее запускается Рэилс-сервер, среди прочего он говорит, на каком порту запускается, например 3000 (http://127.0.0.1:3000/)
# http://localhost:3000/  - соответсвенно локальный адрес для открытия Рэилс приложений
