...
# Но чтобы невозможно было подобрать пароли, шифруя все популярные пароли и создав базу хэшй, то добавляется к паролю еще случайная последовательность символов (Солд) и только потм шифруется, тогда подбирать будет очень сложно
