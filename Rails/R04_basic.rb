...
# 2. далее выполняем миграцию при помощи команды, которая создаст таблицу в БД и БД(если еще не создана).
...
# Экшен - это метод в классе-контроллере, отвечающий за обработчик(get, post итд) какого-либо URL
...
ActionController::Base # (для методов Рзэилс ?) содержит методы params, respond_to итд
...
class HomeController < ApplicationController # контроллер это класс наследующий от главного контроллера
  def index # а экшен/действие это метод в этом классе(отвечающий за обработку запросов)
    ...
    # по умолчанию возвращает(рэндерит) представление с именем экшена из поддиректории с именем контроллера, тоесть тут это будет views/home/index.html.erb
  end
end
...
Rails.application.routes.draw do
  ...
  # Маршруты для работы с моделью Article и контроллером ArticlesController
  resources :articles
  ...
  # 3. resources - маршруты по паттерну REST
