...
# Хелпер - это метод, кторый работает между контроллером и представлением. Нужен чтобы не вставлять дополнительный код в представление:
...
# 2. Сложную kогику в представлениях писать не принято, лучше выносить в хелперы. Представления предназначены для того, чтобы отображать данные. Нехорошо размазывать логику по всему приложению, лучше держать в одном месте
...
# app/helpers/application_helper.rb - главный хэлпер-модуль, в котором обычно пишут свои глобальные хэлперы, использующиеся на многих страницах, относящимся ко многим разным моделям и контроллерам
module ApplicationHelper
  # тут можно писать методы-хэлперы
end

# Для каждого нового контроллера можно создать отдельный хелпер в каталоге /app/helpers/
module ArticlesHelper # хэлпер-модуль для контроллера Articles /app/helpers/articles_helper.rb
  # тут можно писать методы-хэлперы
end
...
# 0. content_tag - универсальный тег, первй параметр имя тега, 2й параметр это контент что будет оборнут в этот тег
content_tag 'span', 'Some text', class: 'some'
...
# URL для контроллера в нэймспейсе (тоесть который обособлен в отжельной папке и в модуле)
form_with model: [:admin, user] do |f|
  # [:admin, user] - URL для адмиского маршрута этого юзера
end
...
puts '                                       Простые кастомные хэлперы'


module ApplicationHelper

  # 1. Кастомный хелпер, возвращающий URL, чтобы отображать какую-то аватарку пользователя если он не ввел URL на свою аватарку (для какого-то приложения в котором юзер имеет свойство с URL аватарки)
  ...
  # 2. Кастомный хелпер, возвращающий тег с иконкой Фонт Ассом
...
# 1. Теперь в представлениях можем использовать этот хелпер и подставлять URL, например в тег img
image_tag user_avatar(@user), alt: 'some'

# 2. Используем хелпер в представлении например для ссылки
link_to fa_icon('trash-o'), question_path(@question), method: :delete, data: {confirm: 'shure?'}



puts '                           provide. Кастомный хэлпер для названия страницы во вкладке'

# provide  - встроенный хэлпер, передающий из встраимого в лэйаут представления, значение в виде параметра или блок с кодом и/или разметкой для дополнительного именованного yield, находящимся в том же представлении, что и главный yield

# (На примере AskIt) Можно на вкладке сайта в браузере, через теги <head> -> <title> добалять не только статическое название сайта, но и динамически добавлять туда название активной, в данный момент, страницы.

# В главном хэлпере app/helpers/application_helper.rb напишем хэлпер-метод, оператор которого будет помещен в layouts/application(helper).html.erb -> <head> -> <title> и будет принимать название конкретной страницы и добавлять его к названию сайта во вкладке браузера:
module ApplicationHelper

  def full_title(page_title="")
    # page_title - параметр с названием конкретной страницы
    if page_title.present?      # если название страницы существует(значение переменной установлено, тут не равно "") ...
...
# Тоесть мы подставляем, например <% provide :page_title, 'Questions' %> в questions/index.html.erb, когда эта страница рендерится в главный yield лэйаута, то в yield(:page_title) передается значение 'Questions', а оно как параметр передается в хэлпер и возвращается нужное название
...
# (На примере AskIt) По умолчанию пункты меню в шапке в лэйаут никак не отображают где мы находимся, напишем для этого хэлперы
...
# 2. Кастомный хэлпер currently_at, чтобы рендерить паршал с меню на страницах. В нашем случае из паршала в представление просто будет передан блок с меню, а потом, когда представление будет подставлено в лэйаут через yield, то через provide блок с меню подставится в соответсвующий второстепенный именованный yeald
module ApplicationHelper
  ...
  def currently_at # Обработку значения параметра, например 'Questions', рассмотрим ниже
    ...
    # render partial:  - говорит о том что рендерим именно паршал, а не полноценный вид, так будет рендерить без лэйаута, а по умолчанию(?? если рендерим не прямо из представления ??) рэндерит с ним. Тк тут подставляем не в лэйаут, а в страницы, которые уже потом подставляются в лэйаут
  end

end
# Последовательность работы:
# 1. Вызывается хэлпер currently_at с целевого представления(тут questions/index.html.erb)
# 2. Хэлпер currently_at(выше) рэндерит в целевое представление паршал с меню shared/_menu.html.erb
# 3. Кода представление уже с подставленным парщалом подставляется в главный yield - код с блоком <% provide :main_menu do ... end %> отправляется в точку <%= yield :main_menu %>
...
module ApplicationHelper
  ...
  def currently_at(current_page='')
    # current_page - параметр с тем значеним что мы передали в обработчик(тут 'Questions')
  ...
  def nav_tab(title, url, options = {}) # будет принимать параметры из обработчика, по той же схеме что и ссылка(название, URL, дополнительные опции в том числе классы)
    current_page = options.delete :current_page # значение текущей страницы(тут 'Questions') из элемента опций под ключем :current_page, заодно удаляем его из хэша options, тк будем использовать этот же options далее
    ...
    link_to title, url, options # передаем измененный хэш options и параметры названия и URL в стандартный хэлпер ссылки и возвращаем его
  end
