...
# Пагинация - разбитие данных, передаваемых в представление, на отдельные страницы
...
# Сгенерирует фаил конфигурации config/initializers/kaminari_config.rb, где можно редактировать параметры пагинации


# questions_controller.rb  - разобьем по страницам вывод вопросов в questions/index.html.erb
def index
  @questions = Question.order(created_at: :desc).page(params[:page]).per(10)
  # page          - метод kaminari который будет разбивать вопросы по отдельным страницам(по умолчаниюпо 25 на странице)
  # params[:page] - папаметр :page определяет какую страницу хочет запросить пользователь - при переключении страниц в URL-адрес добавляется параметр страницы, например: /questions?page=2
  # per(10)       - необязательный метод kaminari, изменяющий колличество сущьностей на страниице(по умолчанию 25)
end
...
# 5. В экшенах контроллеров применим синтаксис pagy:
# questions_controller.rb
def index
  @pagy, @questions = pagy Question.order(created_at: :desc)
  # pagy       - метод создает и возвращает массив из 2х элементов, поэтому помещаем их в 2 переменные
...
# 6. Добавим в questions/index.html.erb специальный хэлпер pagy - pagy_bootstrap_nav для того, чтобы переключаться по страницам со стилями Бутстрап:
pagy_bootstrap_nav @pagy # метод для бутстрап, но есть и другие с разными параметрами
...
# 7. Тк в pagy нет по умолчанию сокрытия меню пагинации если всего одна страница, то сделаем это кастомно на примере questions/index.html.erb, так же вынесем это в кастомный хэлпер pagination для удобства
module ApplicationHelper
  include Pagy::Frontend
