...
puts
puts '                                   Метод в модели - scope синтаксис'

# Различия между scope и обычным методом класса модели в то что scope всегда возвращает relation, а обычный метод класса может вернуть nil, поэтому для метода класса нужны доп проверки или использовать &.
...
# В questions_controller.rb изменим экшен index
...
# Виртуальный атрибут это тот что можно вызывать на экземпляре модели, тоесть существуют в модели, но не существует поля в таблице для него. Нужен например если нужно проверить какието дополнительные данные переданные через форму, которые в отличие от остальных не нужно заносить в БД
...
puts
puts '                             update_column - метод помещающий чтото в БД'

# Можно заполнять БД в модели, при помощи метода update_column
class User < ApplicationRecord
...
# Consern-ы для модели это модули, которые хранятся в директории app/models/conserns отдельными фаилами. Они нужны чтобы выносить код например по определенным задачам или повторяющиеся в рвзных моделях ваомлации в отдельные нэймспэйсы, чтобы модель не была громоздской и чтобы их можно было использовать в разных моделях.

# 1. Создаем необходимые консерны в директории app/models/conserns
module SomeMethod # консерн это модуль
...
# В другой консерн поместим например общие для нескольуих моделей валидации
