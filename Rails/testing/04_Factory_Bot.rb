...
# Фабрика для тестирования - нужна чтобы не создавать в AR объекты для теста вручную, вместо этого создаётся фабрика, которая и будет создавать объекты для теста. Это соответствует принципу DRY
...
# /spec/factories/articles.rb - создадим файл в котором будем создавать фабрику для создания объектов модели Article.
FactoryBot.define do # определяем фабрики
  # factory - метод создает фабрику, принимает ее имя, например article, по умолчанию будет брать соответсвующую имени фабрики модель Article и устанавливать значения в ее свойсва, которые будут описаны в принимаемом блоке
  factory :article do
    # Свойства задаются методами, одноименными с соответсвующими полями модели, а в блоках принимаются значения по умолчанию для них, которые будут присвоены во все объекты, если не изменить их при создании. При создании объектов будет происходить валидация, если она прописана в модели, так что значения должны соответсвовать, если мы хотим ее пройти в тесте
    title { "Article title" }
    text { "Article text" }
  end
end
# Теперь эта фабрика может создавать сущности модели Article с полями title и text
