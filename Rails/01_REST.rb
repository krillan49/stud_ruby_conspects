...
# REST API - соглашение, архитектурный стиль(не протокол как иногда ошибочно называют, тк имеет некую свободу, а протоколы строгие), который сообщает нам набор правил о том как наиболее эффективно общаться клиенту и серверу, используя HTTP и строить свою API так чтобы она была удобной и эффективной.
...
# 5. Кэширование, как засчет заголовков HTTP, так и сторонними решениями, например Redis(GET и POST запрсы могут кэшироваться, а PUT и DELETE не кэшируются). Например какие-то редко изменяемые данные(например список валют) при множестве запросов логичнее не каждый раз брать с сервера, а кэшировать и хранить прямо в браузере, либо при помощи сторонних кэшей на сервере(Рэдис). Это увеличивает скорость и снижает нагрузку на сервер

# Формат обмена данными с REST API может быть любым, но чаще всего это json и иногда xml

# REST API версионирование:
# Если мы меняем API-функционал на обратно-несовместимый, например удаляем метод DELETE для какой-то сущности, то необходимо менять версию, чтобы старые пользователи так и работали со старым API, а новые пользователи(или те кто сами хотят перейти) начинают использовать новую версию
...
# Все URL-ы типа /resourse для всех методов запроса
# ---------------------------------------------------------------------------------------------------------
...
# URL-ы 2х типов /resources и /resources/:id для разных методов запроса
# ---------------------------------------------------------------------------------------------------------
# HTTP Verb     Path                Controller#Action     Хэлпер для URL
# ---------------------------------------------------------------------------------------------------------
...
# /resource/new    /resources/new    /resource/edit   /resources/:id/edit   - дополнительные соглашения, которые напрямую не входят в общее соглашение REST, помогают предварительно сгенерить страницу, как правило форму
