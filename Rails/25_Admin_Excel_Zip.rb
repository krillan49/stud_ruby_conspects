puts '                                  Импорт/экспорт Excel, архивы ZIP'
...
puts '                              Создание xlsx фаилов. axlsx.xlsx шаблоны'
...
puts '                           Загрузка данных из xlsx из архива в БД приложения'
  ...
  class UsersController < ApplicationController
    ...
    def create
      if params[:archive].present?  # проверяем был ли передан архив в форме
        UserBulkService.call params[:archive] # передаем zip архив переданный из формы на обработку в сервисный класс в метод self.call (код там)
...
# б) Создадим базовый/материнский сервисный класс services/application_service.rb от которого будут наследовать другие
