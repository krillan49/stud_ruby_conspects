  ...
  class UsersController < ApplicationController
    def index
      respond_to do |format|
        ...
        format.html do # формат html, пишем в блоке тоже что писали в контроллере по умолчанию, тк html это формат по умолчанию
...
# caxlsx - это гем для генерации файлов Excel (в формате .xlsx, использующем Open XML Spreadsheet). Он позволяет программно создавать Excel-файлы с таблицами, стилями, графиками итд. Это форк и замена axlsx с тем же API(axlsx стал неактивным), он по-прежнему определяет модуль Axlsx, чтобы не ломать совместимость со старым кодом
gem 'caxlsx', '~> 4'

# Пример работы (вне Рэилс):
require 'caxlsx'
package = Axlsx::Package.new # 
workbook = package.workbook
workbook.add_worksheet(name: "Пример") do |sheet|
  sheet.add_row ["Имя", "Возраст"]
  sheet.add_row ["Иван", 30]
end
package.serialize('example.xlsx')


# caxlsx_rails - интеграция Axlsx с Rails, для рендеринга .xlsx в контроллерах и шаблонах (форк axlsx_rails)
...
right_cell = s.add_style(border: Axlsx::STYLE_THIN_BORDER, alignment: { horizontal: :right }) # создаем стиль сразу с двумя ситлями, границей ячеек и выравниваем контента в ячейке по правому краю
...
wb.add_worksheet(name: 'User') do |sheet| # добавляем (?? в документ Excel) новый лист с именем 'User'
  ...
  # ['ID', user.id]              - каждый элемент массива это ячейка в строке таблицы Excel (? тут 'ID' название ряда, а user.id то что будет в следующей строке в этом ряду ??)
...
# 0. rubyXL - гем для считывания xlsx фаилов, тоесть парсинг входящего XLSX-файла (тк caxlsx только для генерации) 
# https://github.com/weshatheleopard/rubyXL
gem 'rubyXL', '~> 3.4'
# > bundle i


# 0. гем для оптимизации запросов при заполнении БД (добавляет множество записей одним запросом). Поддерживает оптимизацию как добавления новых так и обновления существующих записей, что универсально.
# Вместо того, чтобы вставлять или обновлять записи по одному, можно, например собирать их в массивы и выполнять массовую вставку за один запрос. Это улучшит производительность, особенно при большом объеме данных, и сократит нагрузку на базу данных
