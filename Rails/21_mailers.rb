...
class PasswordResetMailer < ApplicationMailer # мэйлеры обычно именуются в единственном числе
  ...
  def reset_email
    ...
    # По у молчинаю использует шаблоны для писем из представлений мэйлера(описаны ниже) с одноименным методу reset_email
  end
end
...
# Но чтобы отправить письмо нужен какой-то почтовый сервер, настройка почтового сервеса, это вещь отдельная и она зависит от того где будет хоститься наше приложение, обычно на хостинге есть свои варианты, так же есть сервисы типа СантГрид и МэилГАн, с помощью которых можно нстроить отправку писем. Но это все для развертывание в продакшене, в девелопмент среде лучше использовать локальный вариант.
