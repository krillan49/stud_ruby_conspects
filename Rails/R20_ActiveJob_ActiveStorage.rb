...
# Некоторые задачи выполняются долго(даже минут 30) и если мы выполняем их синхронно, то у пользователя в браузере произойдет таймаут, браузер прервет выполнение и выдаст ошибку что страница грузится слишком долго

# ActiveJob - это функционал встроенный в Рэилс, который позволяет отправлять задачи предназначенные для выполнения в фоновом режиме различным адапретам(kafka, sidekiq), которые и выполняют эти задачи в фоновом режиме, те управляют очередями, задачами итд

# Тоесть пользователь не ждет пока задача будет выполнена, а может дальше чето делать в приложении и ничего не подвиснет, тк задача будет выполняться в бэкграунде.

# Но в Рэилс нет адаптеров по умолчанию, поэтому нужно специальное решение, которое будет правильным образом выполнять обработку, ставить задачи в очередь, при необходимости выполнять задачи снова, если возникла ошибка итд
...
# Так же если нужно можно запускать sidekiq одновременно с сервером Рэилс, соотв и вывод будет и Рэилс и сайдкик в одной консоли, для этого добавим строку с вркером в Procfile.dev (в нем располагаются инструкции запуска через bin/dev)
# worker: bundle exec sidekiq -q default  # добавим
