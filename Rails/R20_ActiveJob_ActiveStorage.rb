...
# Некоторые задачи выполняются долго и если мы выполняем их синхронно, то у пользователя в браузере произойдет таймаут, а если задача выполняется слишком долго браузер прервет выполнение и выдаст ошибку что страница грузится слишком долго

# ActiveJob - это функционал встроенный в Рэилс, который позволяет отправлять задачи предназначенные для выполнения в фоновом режиме различным адаптерам(kafka, sidekiq), которые будут выполнять эти задачи в фоновом режиме, те будут управлять очередями, задачами итд. Пользователь не ждет пока задача будет выполнена, а может дальше чето делать в приложении и ничего не подвиснет, тк задача будет выполняться в бэкграунде.
