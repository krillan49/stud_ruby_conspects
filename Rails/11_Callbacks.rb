...
# Коллбэк получает текущий объект модели (`self`) в качестве аргумента
...
# Все коллбэки (кроме `after_commit` и `after_rollback`) выполняются только если объект является валидным. Если валидация не прошла, сохранение не произойдет, и последующие коллбэки не будут вызваны.
...
# Принято как можно меньше ответственности перекладывать на коллбэки и перемещать логику например в сервисные объекты, вызванные из контроллеров. Чрезмерное использование коллбэков может сделать код сложным для понимания и отладки. Рассмотрите возможность использования сервисных объектов или других паттернов проектирования, если логика становится слишком сложной.
...
# a) Валидационные колбэки - вызываются, когда запись проверяется на валидность напрямую через методы valid? (или его псевдоним validate) или invalid?, или косвенно через методы create, update или save

# before_validation - вызывается перед выполнением валидации модели (автоматически при выполнении `save`, `create`, `update`, `valid?`).
before_validation :titleize_name # выполняется перед проверкой валидации

# after_validation - вызывается после выполнения валидации модели, независимо от того, была ли она успешной или нет. Полезно для выполнения каких-либо действий, зависящих от результатов валидации (например, логирование ошибок)
after_validation :log_errors     # выполняется после проверки валидации, но до сохранения в БД


# b) Колбэки сохранения - вызываются когда запись передается(сохраняется) в БД с помощью методов create, update или save. Они вызываются до, после и во время сохранения объекта:

# before_save - вызывается перед сохранением записи в БД, как при создании, так и при обновлении, перед выполнением операций `create` или `update`, но только если валидация прошла успешно.  Полезно для изменения атрибутов перед записью, например, установка `updated_at`, создание `slug` итд.
before_save :hash_password # выпоняется перед сохранением (создание или обновление) в БД

# around_save - обертывает процесс сохранения записи. Позволяет выполнять код до и после сохранения, а также предотвратить сохранение, если это необходимо. Вместо выполнения `save`, вызывается этот коллбэк. Внутри него нужно вызвать `yield`, чтобы продолжить процесс сохранения (если вы хотите его выполнить).
around_save :log_saving    # выпоняется во время сохранения (создание или обновление) в БД

# after_save - вызывается после сохранения записи, как при создании, так и при обновлении, после успешного выполнения операций `create` или `update`. Обычно лучше использовать `after_create` и `after_update` вместо `after_save` для более четкой логики.
after_save :update_cache   # выпоняется после сохранения (создание или обновление) в БД


# c) Колбэки создания - вызываются когда запись впервые сохраняется в БД. Они срабатывают при сохранении новой записи с помощью методов create или save. Они вызываются до, после и во время создания объекта.

# before_create - вызывается перед созданием новой записи, только перед выполнением операции `create`, и только если валидация прошла успешно. Полезно для установки значений по умолчанию для новых записей
before_create :set_default_role  # выпоняется перед сохранением (только создание нового объекта) в БД

# around_create - обертывает процесс создания новой записи. Вместо выполнения `create`, вызывается этот коллбэк.
around_create :log_creation      # выпоняется во время сохранения (только создание нового объекта) в БД

# after_create - вызывается после создания новой записи, только после успешного выполнения операции `create`. Полезно для отправки уведомлений, создания связанных объектов итд.
after_create :send_welcome_email # выпоняется после сохранения (только создание нового объекта) в БД

# after_commit :on => :create - вызывается после завершения транзакции БД, в которой была создана запись. `after_commit` выполняется только если транзакция была успешно закоммичена. Только после успешного завершения операции `create` внутри транзакции. after_commit выполняется после `after_create`

# after_rollback :on => :create - вызывается после отката транзакции БД, в которой создавалась запись. `after_commit` выполняется только если транзакция была успешно закоммичена. Только после неуспешного завершения операции `create` внутри транзакции. after_rollback выполняются после `after_create`


# d) Колбэки обновления - вызываются всякий раз, когда уже существующая запись сохраняется в БД Они вызываются до, после и во время обновления объекта

# before_update - вызывается перед обновлением существующей записи, перед выполнением операции `update`, и только если валидация прошла успешно. Полезно для отслеживания изменений, установки `updated_at` итд
before_update :check_role_change # вызывается до обновления существующего объекта

# around_update - обертывает процесс обновления существующей записи. Вместо выполнения `update`, вызывается этот коллбэк
around_update :log_updating      # вызывается после обновления существующего объекта

# after_update - вызывается после обновления существующей записи, только после успешного выполнения операции `update`. Полезно для инвалидации кэша или уведомлений об изменении
after_update :send_update_email  # вызывается во время обновления существующего объекта

# after_commit :on => :update - вызывается после завершения транзакции БД, в которой была обновлена запись, только после успешного завершения операции `update` внутри транзакции. Выполняется после `after_update`

# after_rollback :on => :update - вызывается после отката транзакции БД, в которой обновлялась запись, только после неуспешного завершения операции `update` внутри транзакции. Выполняется после `after_update`


# 2. Уничтожение объекта. Список в том порядке, в котором они вызываются если существуют одновременно:

# before_destroy - вызывается перед удалением записи, перед выполнением операции `destroy`. Полезно для проверки прав доступа, удаления связанных объектов или предотвращения удаления записи. Если коллбэк вернет `false`, запись не будет удалена.
before_destroy :check_admin_count

# around_destroy - обертывает процесс удаления записи. Вместо выполнения `destroy`, вызывается этот коллбэк.
around_destroy :log_destroy_operation

# after_destroy - вызывается после удаления записи, только после успешного выполнения операции `destroy`. Полезно для очистки зависимостей или уведомлений об удалении.
after_destroy :notify_users

# after_commit :on => :destroy - вызывается после завершения транзакции БД, в которой была удалена запись, только после успешного завершения операции `destroy` внутри транзакции. Выполняется после `after_destroy`

# after_rollback :on => :destroy - вызывается после отката транзакции БД, в которой удалялась запись, только после неуспешного завершения операции `destroy` внутри транзакции. Выполняется после `after_destroy`
...
# 4. Коллбеки транзакции

# after_commit - вызывается после завершения любой транзакции БД, которая затронула запись, после успешного завершения любой операции (`create`, `update`, `destroy`) внутри транзакции.
after_commit :some

# after_rollback - вызывается после отката любой транзакции БД, которая затронула запись, после неуспешного завершения любой операции (`create`, `update`, `destroy`) внутри транзакции.
after_rollback :some

# Транзакции: Операции сохранения (create, update, destroy) обычно выполняются в транзакциях БД. Это гарантирует целостность данных. `after_commit` и `after_rollback` используются для выполнения действий, которые должны быть выполнены только после того, как транзакция успешно закоммичена или откачена.

# Когда использовать `after_commit` и `after_rollback`
# - Отправка электронной почты:  Отправка электронной почты должна происходить только после того, как транзакция успешно закоммичена. Если транзакция будет отменена, письмо отправлять не нужно.
# - Интеграция с внешними сервисами:  Если вы интегрируетесь с внешним сервисом, нужно выполнять действия с этим сервисом только после того, как транзакция успешно закоммичена.
# - Если ваша операция влияет на кэш
...
# touch - если вызван метод `touch` на связанной модели, это вызовет коллбэки `before_update` и `after_update` на этой связанной модели.

# ?? Коллбэки выполняются в том порядке, в котором они определены в классе модели. Дополнительно помимо порядка в зависимости от коллбека ??



puts '                                      Комбинирование колбэков'

# Часто требуется комбинация колбэков. Например, нужно отправить письмо после создания пользователя, но только если это новый пользователь, а не обновляемый. А при обновлении пользователя уведомить администратора, если были изменены важные данные. В этом случае можно использовать вместе колбэки after_create и after_update.
...
# Методы `create!`, `update!`, `destroy!` бросают исключение, если происходит ошибка при сохранении/обновлении/удалении. Это может повлиять на выполнение коллбэков `after_commit` и `after_rollback`
...
puts '                                        Остановка выполнения'

# Если коллбэк `before_save`, `before_create`, `before_update` или `before_destroy` возвращает `false`, выполнение операции сохранения/создания/обновления/удаления будет остановлено. Валидация при этом должна пройти успешно

# В случае `around_*` коллбэков, чтобы продолжить выполнение стандартного действия, необходимо вызвать `yield` внутри коллбэка.  Если `yield` не вызван, операция будет остановлена.
