puts '                           callbacks(функции обратного вызова) для модлей'
...
# Колбэки в какой-то степени заменяют код относящийся к моделям, который мы бы иначе писали руками, например в контроллерах (тк сохраняем и обновляем сузности мы вызывая методы в контроллерах)

# В callbacks можно переавать простую логику, простые валидации, асинхронные задачи (jobs) так чтобы они не зависели друг от друга не обращадись к сущностям других модделей и тем более не зависили от других слоев вне модели. Принято как можно меньше ответственности перекладывать на коллбэки и перемещать логику например в сервисы, объекты, вызванные из контроллеров
...
before_validation :titleize_name # перед проверкой валидации
after_validation :log_errors # после проверки валидации но до сохранения в БД
...
before_save :hash_password # перед созданием или обновлением объекта
...
before_create :set_default_role # перед созданием, но не перед обновление существубющего
...
class User < ApplicationRecord
  ...
  before_save :downcase_nickname # вызовет метод перед соранением как новой так и обновлением существующей записи в БД
