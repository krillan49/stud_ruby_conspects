...
class ItemsController < ApplicationController
  ...
  rescue ActiveRecord::StaleObjectError # Обработаем ошибку редиректом(можно еще добавить и флэш-сообщение)
    redirect_to item_path(@item)
  end
...
# Подходит например для того чтобы изменить какоето значение, предполагающее изменение многими пользователями, например добавить сумму на счет или изменить чтото в статистике в зависимости от действий пользователя.
...
# 3б. Локинг полный для следующих пользователей вызывает ошибку, тоесть сработает так же как и оптимистик:
# Контроллер такойже но изменим экшен update:
class Item2sController < ApplicationController
