...
# Колбэки экшна наследуются, поэтому, если добавить их в ApplicationController, он будет запускаться в каждом контроллере.



puts '                                           before_action'

# before_action  -  метод, который регистрирует "before"-колбэки экшена, он принимает в параметры имя метода в виде символа и исполняет этот метод в области экшена, перед тем как запускается каждый экшен контроллера. Экшены, перед которыми будет выполнен метод, так же передаются в параметры (по умолчанию это все экшены). Нужен, например чтоб запускать повтопяющийся в нескольких экшенах код.

# Колбэки экшна "before"  могут прерывать цикл запроса.

# Если колбэк экшна "before" рендерит или перенаправляет, экшн контроллера не запустится. Если есть дополнительные колбэки экшна в очереди, они также будут отменены.
...
class QuestionsController < ApplicationController
  ...
  # Можно использовать несколько фильтров в одном контроллере, порядок их важен если предыдущий создает объекты, которые использует следующий фильтр:
  ...
  # Все записи внутри экшенов ниже равноценны, перед каждом исполняется метод set_question! определяющий переменную экземпляра, которая теперь доступна в экшене
  def show
...
# skip_before_action - метод, который отменяет действие какого-то фильтра, для всех или определенных экшенов данного контроллера, соответсвенно принимает имя фильтра и имена экшенов, для которых нужно деактивировать этот фильтр. По умолчанию деактивирует фильтр для всех экшенов.


# Обычно "before"-колбэк нужен, чтобы чтобы отфильтровать авторизованных пользователей для запуска экшна.
...
# Но колбэк добавлен в ApplicationController, и все контроллеры наследуют его. Тоесть неавторизированный пользователь, даже не сможет зарегистрироваться, соответсвенно не все контроллеры или экшны должны требовать его. Тут стоит деактивировать запуск этого фильтра перед определенными экшнами LoginsController с помощью skip_before_action:
...
# Теперь, экшны LoginsController new и create будут работать как раньше, без требования к пользователю быть зарегистрированным.



puts '                                     after_action и around_action'

# Колбэки экшна after и around - запускаются после того, как экшн контроллера был выполнен, или "и до, и после".

# Колбэки экшна "after" регистрируются с помощью after_action. Они похожи на колбэки экшна "before", но поскольку экшн контроллера уже был запущен, у них есть доступ к данным отклика, которые будут отосланы клиенту. Колбэки экшна "after" не могут остановить экшн от запуска. Колбэки экшна "after" выполняются только после успешного выполнения экшна контроллера, но не при возникновении исключения в цикле запроса.
...
# Колбэки экшна "around" также оборачивают рендеринг. В частности, в вышеуказанном примере, если вью сама начнет считывать из базы данных (например через скоуп), она это осуществит внутри транзакции, предоставив, таким образом, данные для предварительного просмотра.
...
# 2. Использовать класс для управления колбэком экшна. Это полезно для сложных задач, которые не сложно осуществить предыдущими способами из-за трудности читаемости и/или повторного использования.

# Класс должен реализовывать метод с тем же именем, что и колбэк экшна, поэтому для колбэка экшна before_action класс должен реализовать метод before, и так далее. Метод around должен иметь yield для выполнения экшна.
