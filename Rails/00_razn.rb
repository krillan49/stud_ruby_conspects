...
# b) Когда приходит http-запрос (например от браузера) тоесть специальный веб-сервер(например nginx) который этот запрос обрабатывавет  использует часть данных запроса, чтобы определить, что с ним дальше делать
...
# 2. Внутри Реилс-приложения (Тоесть Руби-процесса):
# a) Чтобы понять, что делать с запросом Рэилс сперва обрабатывает запрос, парсит, достает заголовки и как только он понял что запрос валидный, то обращается в routes.rb, который мы конфигурируем и в соотвествии с этими правилами становится понятно какому контроллеру и экшену передать этот запрос. Если контроллера и экшена удовлетворяющего этот запрос нет, ро Рэилс передает в браузер какую-то ошибку
# b) Далее Рэилс создает новый экземпляр целевого контроллера (создается для каждого запроса новый) и от него запускают соответсвующий метод(экшен), в который передается запрос. Экшен при своем исполнении взаимобдействует с моделями, модель сваязывается с БД(которая как бы вне Рэилс приложения отдельно) и возвращает данные контроллеру и он ищет нужнуюю вьюху(шаблон)(заданную явно или по умлчанию), когда вьюха найдена то специальный код формирует на основее ее и данных из модели html-страницу, json или что-то еще, что будет отправлено обратно nginx-у, а он соответсвенно передост дальше браузеру в виде http-ответа

# Что из перечисленного пишется программистом:
# routes.rb
# контроллеры
# модели и миграции
# шаблоны

# Что из перечисленного программист не трогает (магия Рэилс):
