puts '                               Авторизация(роли для пользователей). Enum'

# (Если приложение задеплоить в продакшен – то создать пользователя это - Либо делать скрипт seeds.rb либо руками через консоль. Это однократное действие, так что это норм - назначили первого админа, он потом назначает других. Но возможно имеет смысл сделать валидацию в духе "нельзя удалить последнего админа")
...
class User < ApplicationRecord
  ...
  # _suffix: :role - добавляем суффикс(не обязательно) "role"(название любое) к методам проверки роли

  # ...
end
# Посмотрим в консоли (> rails c):
User.roles      #=> { basic: 0, moderator: 1, admin: 2 }   # обращвемся к модели и получаем от enum хэш ролей
u = User.first  #=> #<User:0x000001cd7ed4adf8 id: 1, ... , role: "basic">  # enum автоматически конвертировал 0 из БД в "basic"
u.role          #=> "basic"  # метод модели role возвращает роль из enum сопоставленный с числом из БД
u.basic_role?   #=> true     # метод проверяет на конкретную роль. Имя метода собирается из компонентов заданных в enum, а именно названия роли и суффикса если он есть(если нет суффикса было бы просто u.basic?)
u.admin_role?   #=> false
u.role = :admin #=> :admin   # метод role так же имеет сеттер, чтобы назначать роль пользователю (это нужно сохранять отдельно чтоб записать в БД так же как и любое другое изменение в БД)
u.admin_role?   #=> true
...
class User < ApplicationRecord
  ...
  # !admin_edit - если это поле не установлено(false), то вылидацию correct_old_password делать нужно, а если установлено(true), значит это админ и данную валидацию не проводим
...
# 2. Подключим Pundit в application_controller.rb:
...
class GuestUser
  ...
  def method_missing(name, *args, &block)
    # *args  - например когда проверяем авторстсво записи или коллекции записей?
    # &block - хз зачем, просто на всякий случай на будущее ??
...
# comments_controller.rb - сделаем авторизацию прямо в экшенах, без фильтра и отельного метода authorize_answer!, тк всего 2 экшена
...
puts
# Политика для админ-юзеров(тк есть одельный нэймспейс админов):
