...
# Рэилс автоматически отрисовывает Лэйаут (макет) layouts/application.html.erb
# HTML-код лэйаута отрисовывается пока не встретит ключевое слово yield и подставляет(отрисовывает там шаблон/вью)
# Стоит с отсторожность использовать инстанс переменные объектов в лэйаут, тк он может использоваться с теми экшенами контроллева в которых они не определяются
