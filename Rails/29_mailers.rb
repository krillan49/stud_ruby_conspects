...
# (AskIt). Создадим отдельный маршрут, контроллер и представление, где пользователь сможет, без входа на свой профиль, (где он и так может все поменять) ввести свой имэйл адрес и нажать кнопку "сбросить пароль" (если например он забыл свой пароль). После отправки этой формы пользователю на почту придет уникальная ссылка, в ней будет специальный защитный токен, после сброса данный токен становится невалидным, а у пользователя появляется новый пароль.
...
# 2. Создадим кнопку для спроса пароля и директорию для представлений сброса пароля /password_resets
# sessions/new.html.erb - добавим кнопку "забыл пароль" с которой и вызовется представление с формой ниже
...
class ApplicationMailer < ActionMailer::Base
  default from: 'admin@askit.com' # тут мы можем написать/изменить от кого(мы/сервер) будут письма приходить (пофиг что ??)
...
# mailers/password_reset_mailer.rb - создадим новый мэйлер, через который мы будем отправлять почту с данными по смене пароля
class PasswordResetMailer < ApplicationMailer # мэйлеры обычно именуются в единственном числе
  # Создадим метод при помощи которого будет отправляться письмо с токеном(название любое)
  def reset_email
    @user = params[:user]
    # :user - сущность юзера в парамс установлена при вызове данного метода из контроллера PasswordResetsController#create при помощи with(user: @user)
    ...
    # По у молчинаю использует шаблоны для писем из представлений мэйлера(описаны ниже) с названиями одноименными методу отпавки письма(тут reset_email), тоесть так же как с экшенами контроллера и представлениями
  end
end
...
class PasswordResetsController < ApplicationController
  ...
  # А где new ?? (работает и без указания экшена втф ??)

  # Экшен create тут будет отправлять инструкции для сброса пароля
  def create
    @user = User.find_by email: params[:email] # находим юзера по имэйлу, указанному пользователем в форме

    if @user.present? # если юзер нашелся в БД
      # Вызываем наш метод reset_email мэйлера PasswordResetMailer для отправки почты:
    ...
    flash[:success] = t '.success' # в любом случае success, чтоб злоумышленники не угадывали пароли
    redirect_to new_session_path   # на страницу логина
  end
end


# 5. Создадим шаблоны письма. Для этого в представлениях создадим дирректорию /password_reset_mailer с представлениями, только представлениями не для контроллера, а для мэйлера, которые собственно и будут рендерить письма. Называем представления точно так же как и имя метода(reset_email) в мэйлере, который отправляет письмо, чтобы метод вызывал одноименное представление по умолчанию(суть такая же как и с именами экшенов контроллера):
...
# Чтобы отправить письмо нужен какой-то почтовый сервер, настройка почтового сервера, это вещь отдельная и она зависит от того где будет хоститься наше приложение, обычно на хостинге есть свои варианты, так же есть сервисы типа СантГрид и МэилГАн, с помощью которых можно настроить отправку писем. Но это все для развертывание в продакшене, в девелопмент среде лучше использовать локальный вариант.
...
# Нам неоходимо сгенерировать специальный токен, который мы передадим в письме на почту и потом, получив его обратно в ответе -  сможем идентифицировать, что это тот-же пользователь и соответсвенно имеет право на смену пароля, а не какойнить злоумышленник.
...
# 2. Напишем метод генерации токена. Можно было бы сделать и в модели user.rb, но лучше создадим новый консерн моделей concerns/recoverable.rb и создадим в нем собственно метод set_password_reset_token (код там) который будет создавать токен в экшене create, чтобы отправить его в ссылке в письме
...
class PasswordResetsController < ApplicationController
  ...
  # Если мы изменим токен в адресе то метод set_user сменить пароль недаст
  def edit
