...
# Частичное представление или partial/паршал - это такое представление которое рендерится само по себе, но не вставляется в лэйаут, нужны для того чтоб вставлять кусочки разметки на разных страницах
...
# Флэш сообщения передаются в сессию и появляются только один раз и при перезагрузке страницы их уже не будет. Удобны, например для оповещения пользователя о создании, изменении или удалении сущности
...
class QuestionsController < ApplicationController
  ...
  def create
    ...
    if @question.save
      ...
      # Если заново отрендерить форму, то к флэш сообщению нужно применить метод now, чтобы при перезагрузке страницы флэш-сообщение не отобразилось еще раз
