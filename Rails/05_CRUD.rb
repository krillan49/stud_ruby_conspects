...
# (?? Потом вынести валидации в отдельный фаил, гдето рядом с коллбеками тк это близкие темы ??)
...
# Валидация производится (если назначена) когда применяется метод save, create, update. Если валидация не проходит, то create возвращает объект все поля которого равны nil.
# Если применить к объекту методы valid?, save или update то вернет false если существующие валидаци не пройдены, либо true, если валидации прошли
...
# partial/паршал (Частичное представление) - это представление которое не вставляется в макет(лэйаут) через yield, а рендерится само по себе, чкрез хелпер "render", например в коде полноценной вьюхи, либо в какой-то точке лэйаута отличной от yield. Нужны для того чтоб вставлять одни и теже кусочки разметки в разные вьюхи
...
render partial: 'contacts/contact', contact: @contact
render 'contacts/contact', contact: @contact
render 'contact', contact: @contact # аналог того что выше (если представление и паршал в одной директории)
...
# shared (или partials ?? устарело ??) - ?общепринятое название? для дирректории для общих паршалов, тоесть используемых во вьюхах разных контроллеров
...
class QuestionsController < ApplicationController
  ...
  def create
    ...
    if @question.save
      ...
      # Вариант 2 (прямо в редиректе передать через ключ хэша)
      ...
      # now - метод чтобы флэш сообщение отрисовывалось только при одном запрпосе (текущем), а не при 2х как по умолчанию, тк иначе при перезагрузке страницы флэш-сообщение отобразится еще раз
      render :new
    end
  end
  # ...
