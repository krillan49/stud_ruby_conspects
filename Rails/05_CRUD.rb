...
# Флэш сообщения передаются в сессию (при помощи механизма куки) и появляются только один раз (при следующем запросе, что одходит под редирект) и при перезагрузке страницы их уже не будет. Удобны, например для оповещения пользователя о создании, изменении или удалении сущности
