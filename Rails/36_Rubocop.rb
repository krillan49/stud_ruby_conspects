...
# В итоге выдаст список стилистических ошибок(каждая такая проверка называется 'cop') с их описаниями, а так же их колличество и сколько можно исправить автоматически
...
# Rails/FilePath  - ошибка путей в некоторых записях. Есть 2 варианта записи через слэш slashes('some/some')(значение по умолчанию) или отдельно arguments ('some', 'some'). Установим в .rubocop.yml, что хотим чтоб правильно было отдельно. Какие данные и как прописывать в yml смотрим в описании копов в доках
