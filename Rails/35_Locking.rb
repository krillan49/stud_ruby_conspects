...
# Locking - это временная защита записи в таблице БД от одновременного множественного доступа/изменения разными пользователями. Например 2 пользователя могут попытаться отредактировать одну и ту же запись с разницей в несколько секунд (соответсвенно 2й не в курсе редакции от 1го), чтобы защититься от этого и сохранить в БД только одно изменение и нужен locking
