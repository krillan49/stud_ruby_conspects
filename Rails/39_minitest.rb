...
# * fixtures/                       - Fixtures/фикстуры (если не используешь FactoryBot) это способ создания макетов данных для использования в тестах, чтобы вне приходилось использовать «реальные» данные. Они хранятся в fixtures каталоге.
# * При первом создании задания jobs также будет создан каталог для ваших тестовых заданий

# * test_helper.rb                  - содержит конфигурацию по умолчанию для тестов. Все методы, добавленные в этот файл, также доступны в тестах, когда этот файл включен. Используй test_helper.rb для подключения FactoryBot, очистки базы и других настроек
# * application_system_test_case.rb - конфигурацию по умолчанию для тестов вашей системы


# Не бойся писать мелкие короткие тесты — они легче читаются и изолируются.

# Для UI/system тестов включи selenium_chrome_headless, если нужен реальный браузер
...
# Если используешь Devise::Test::IntegrationHelpers или какие-либо кастомные хелперы, их тоже можно подключить в ApplicationSystemTestCase.



puts '                            Настройка FactoryBot (если используется)'
...
# skip в Minitest выполняет ту же роль, что и pending в RSpec — помечает тест как пропущенный
require "test_helper"
...
# Утверждения — это фундаментальные блоки тестирования, которые выполняют проверки, чтобы гарантировать, что все идет по плану

# С minitest можно добавлять собственные утверждения. Это делает, например Rails

# [msg] - далее это необязательное строковое сообщение, которое вы можете указать, чтобы сделать сообщения о неудачах теста более понятными.
...
assert_not_predicate(obj, predicate, [msg])	              # obj.predicateэто ложно, например assert_not_predicate str, :empty?

assert_error_reported(class) { block }	                  # ласс ошибки был сообщен, например assert_error_reported IOError { Rails.error.report(IOError.new("Oops")) }
