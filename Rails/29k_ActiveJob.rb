...
# ActiveJob - это интерфейс (фрэймворк) встроенный в Рэилс для работы с фоновыми задачами, который позволяет отправлять задачи различным бэкендам/адаптерам(kafka, sidekiq), которые будут выполнять эти задачи в фоновом режиме (?? тоесть независимо от обработки запроса пользователя ??), те будут управлять очередями, задачами итд. Пользователь не ждет пока задача будет выполнена его запросы обрабатыватся и может дальше что-то делать в приложении и ничего не подвиснет, тк задача будет выполняться в бэкграунде
