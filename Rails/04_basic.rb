...
# Controller - это специальный класс, который часто взамодействует с какой-либо моделью, он находится в своем фаиле и отвечает за обработку запросов с пришедших с какого-то URL адреса и возврат представлений браузеру.
...
ApplicationController  # /app/controllers/application_controller.rb (для наших кастомных методов ?) Не имеет своего маршрута. Методы и другие параметры из него унаследуют все наши контроллеры
