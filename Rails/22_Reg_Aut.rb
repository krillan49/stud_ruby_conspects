...
# Сессия - это абстрактное поняте, реализуемая при помощи механизма куки
...
# session - объект похожий на хэш, для удобного чтения и записи значений сессии в Рэилс-приложении (для каждого пользователя разный). Доступиен во всех экшенах контроллера
session['key'] = 'value'
