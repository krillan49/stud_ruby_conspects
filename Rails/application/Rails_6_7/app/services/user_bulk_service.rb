class UserBulkService < ApplicationService
  ...
  def call # основной метод для обработки архива (вызывается в application_service.rb)
    Zip::File.open(@archive) do |zip_file| # Открываем zip архив и передаем открытый фаил в параметр zip_file
      zip_file.glob('*.xlsx').each do |entry| # Перебираем каждый фаил xlsx, который есть в архиве (на случай если их много)
        # Вариант 1 (медленный): при помощи итератора и записи в БД кажого пользователя отдельно (если много зпписей то будет оч медленно)
        u = users_from(entry)
        u.each do |user|
          # ...
        end

        # Вариант 2: оптимизировать при помощи гема activerecord-import, и выполнять все записи одним запросом (валидации будут сделаны). У гема activerecord-import есть еще раздые доп опции, которые можно посмотреть в доках
        User.import users_from(entry), ignore: true
        # import            - метод гема activerecord-import, принимает (тут) массив из пользователей и записывает их в БД одним запросом
        # users_from(entry) - наш кастомный подметод(ниже) создает массив пользователей на основе xlsx фаила из entry (распарсит по ячейкам)
  ...
  def users_from(entry)
    ...
    sheet.map do |row| # на основе распарсенных данных сделаем массив
      ...
      # Далее на основе ячеек создадим объекты новых юзеров, но только в памяти, не сохраняя в БД
