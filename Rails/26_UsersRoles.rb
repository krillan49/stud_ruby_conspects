...
# в СУБД Посгресс есть свои enum со спец типом данных для БД:
...
# При создании экземпляра юзера, enum автоматически конвертирует 0 из БД в заданное нами "basic":
u = User.first  #=> #<User:0x000001cd7ed4adf8 id: 1, ... , role: "basic">
# role - метод экземпляра модели, возвращает роль из enum сопоставленную с числом из БД
u.role          #=> "basic"
# Генерируются автоматически методы экземпляра, что проверяет на конкретную роль. Имя метода собирается из компонентов заданных в enum, а именно названия роли и суффикса если он есть(если нет суффикса было бы просто basic?, admin? итд)
u.basic_role?   #=> true
u.admin_role?   #=> false
# Метод role так же имеет сеттер, чтобы назначать роль пользователю (это после нужно сохранять, чтоб записать в БД так же как и любое другое изменение в БД)
u.role = :admin #=> :admin
u.admin_role?   #=> true
