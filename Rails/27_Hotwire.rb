...
# а. questions(turbo)/index.html.erb - изменим кнопку вызова формы, чтобы она отправляла запрос на new в формате турбострима и экшен рендерил форму на страницу
...
# ?? Почему для формы вопроса используем turbo-frame тег, а в лэйауте обычный div - тк для формы хотим чтобы тег появлялся только при турбо стриме, а для флэша любом случае ??
...
# 4. Динамическое редактирование вопросов, тоесть чтобы при нажатии кнопки "Edit" появлялась форма редактирования в точке вопроса, вместо него и потом вопрос на странице динамически обновился
...
# 2. Исправим ссылку-заголовок на вопрос. На странице вопроса, добавим тот же рендер вопроса, что и на странице всех вопросов, вместо отдельного кода, тк так заодно будет работать турбофрэйм вызывающий форму редактирования.
...
puts
puts '                                 Фрэйм внутри фрэйма. Turbo Frame с Pagy'
...
# 1. Динамическое добавление, редактирование и удаление ответов на странице questions(turbo)/show.html.erb:
