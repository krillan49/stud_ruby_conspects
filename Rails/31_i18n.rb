...
puts '                            Мнение о локализации из чатика(потом разобрать)'

# ну я обычно придерживаюсь таких правил:
# — есть общая локаль (core.en.yml, core.de.yml и т.п.) — там хранятся слова общего назначения, всякие yes, no, agree, cancel, search и в том же духе. слова ,которые встречаются в нескольких местах приложения. в системе они используются напрямую, например I18n.t('seacrh') или даже t(:search) во вьюшках
# — как вариант, можно для core слов использовать просто en.yml, а то, что изначально в нем, перекинуть в другие файлы
# — есть файлы view (views.en.yml, views.de.yml ... ) — там именно для вьюшек конткретных переводы. то есть, один файл на все вьюшки. в самих вьюшках использую синтаксис вида t('.add_to_cart'), потом смотрю, где он ждет локаль и создаю там ключи. если файл разрастается до неудобного состояния — делю, критерии разные в разных проектах
# — мейлеры в отдельных локалях, ну это понятно и так
# — для моделей активно пользую файлы activemodel.en.yml, activemodel.de.yml и т.д.  — там имена и атрибуты

# ГЛАВНОЕ — все ключи (и вложенные  подключи в том числе!) должны быть упорядочены по алфавиту.  Это очень важно, тогда поддержка не будет делать нервы.

# в принципе, этого достаточно, чтобы не потонуть в аду локализации

# Я бы еще добавил, что переводы стоит хранить полными предложениями, при необходимости передавая в них параметры. Во многих языках порядок слов разные и собрать одно и то же предложение из кусков - не получится.
# И если нужен где-то поиск по именам или описаниям, то лучше сразу хранить переводы в базе данных или эластике
