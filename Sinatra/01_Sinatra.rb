...
# (!! Винести отдельно. Примерами можно код с рубикодом созданием пересенных, операторами, циклами итд)
...
# 1. <% любой_руби_код %>     -    строка Руби кода будет выполнена, но ничего не будет вставлено в HTML-страницу.
# 2. <%= любой_руби_код %>    -    строка Руби кода будет выполнена и ее возврат будет вставлен в HTML-страницу в месте где стоит эта вствка
...
# Вариант вывода из БД 1(грязный c html в основном фаиле)
...
configure do
  ...
  # создаем таблицу парикмахеров для селектора в форме
...
# redirect - метод отправляет HTTP-заголовок для перенаправления клиента на заданный URL-адрес, передаваемый аргумент должен быть полным URL-адресом с хостом (например http://example.com/path, а не просто /path). Если метод не получает аргумента, то он будет перенаправлять на корневую страницу ("/").

# to - метод преобразует путь в полный URL-адрес вашего приложения Sinatra. to('/path') #=> http://yoursinatraapp/path.
...
# redirect to не сохраняет при переходе переменные экземпляра из материнского обработчика, тк происходит новый гет-запрос на стороне браузера, поэтому например для @error нужно не редиректить, а возвращать представление.
...
post '/details/:post_id' do # универсальный пост обработчик(тут обрабатывает POST-запросы с URL формы /details/:post_id, которые мы генерируем в leprosorium_details выводя post_id из колонки в таблице постов из БД)
  ...
  # post_id для того чтобы знать к какому посту коментарий
