...
get '/' do # get - метод(запуск), он принимает 2 аргумента: 1й - это адрес('/'), 2й - это лямбда(do...end); get('/', lambda)
  ...
  return 'Hi'  # Метод возвращает просто текст на корневую страницу в пустой html(?) фаиле. return не обязателен как и в любом другом методе
end
...
# создаем в views HTML-страницу, но расширение у нее будет не .html а .erb например index.erb(название любое) -> пришем в нем необходимый нам HTML-код например <h1>Hi</h1> -> мы можем подключить это представление/view/HTML-код в нашу программу через синтаксис в котором расширение(erb) будет методом, а имя фаила(a_index) в виде символа - его параметром
require 'sinatra'
...
post '/' do # в пути пишется тоже самое('/') что и в значении атрибута action в теге формы в .erb фаиле
  login0 = params[:aaa] # локальная переменная может использоваться только в теле запроса, например для интерполяции ее значения в другую переменную или наполнения БД. Если поместить ее в представление, то выдаст ошибку
...
get '/contacts' do # адрес который будет открываться по ссылке из welcome.erb(<a href="/contacts">Contacts</a>). Тк ссылка активирует переход соотв гет-запрос, соотв ему нужен и обработчик
...
# в комментах для открытия и использования базы данных предлагали глобальную переменную с $(проверено - работает) , но тут сделаем с локальной переменной и будем вызывать каждый раз объект базы данных
...
# before (в sinatra) - каждый раз исполняет код в теле метода before перед(в теле запроса, гдето там за кадром??) любым запросом(get/post/...) - соотв код(напр переменные) из before будет доступен во всех обработчиках и соотв представлениях ими возвращамых. Удобно если один и тот же код необходимо использовать в нескольких запросах и возвращаемых ими представлениях
...
def init_db
	# @db = SQLite3::Database.new './DBS/leprosorium.db'  # присваиваем базу данных в переменную с @ чтобы она работала в видах и обработчиках - чтобы не писать init_db каждый раз, а только в before. Тк не получится использовать локальную переменную, тк метод возвращает(втч и before) последнее значение и его пришлось бы присваивать в переменную каждый раз, а @переменные достаточно определить в теле метода и они будут работами в области вызвавшей метод.
	@db.results_as_hash = true
end
...
get '/about' do # ... соответсвенно этот обработчик принимает гет запрос
  "Это страница о нас"
end
...
# Похоже метод redirect to не сохраняет при переходе переменные экземпляра из материнского обработчика, поэтому например для @error нужно возвращать представление.
...
get '/details/:post_id' do # добавляем в адрес :id(название любое) теперь Синатра будет знать что это идентификатор, соответсвенно данный обработчик становится универсальным для любых значений :id. Теперь соответсвенно при переходе на адрес /details/некое_значение (не важно на адрес этот перешли ссылкой(например из предствления(тут leorosorium_index)) или мы просто пишем значение в адресной строке наугад) будет открыта эта страница.
...
post '/details/:post_id' do # универсальный пост обработчик(тут обрабатывает запросы со страниц /details/:post_id, ссылки на которые мы создаем в leprosorium_details выводя post_id из колонки в таблице постов из БД)
  ...
  redirect to ('/details/' + post_id)  # перенаправляем на страницу поста(get '/details/:post_id')
end
