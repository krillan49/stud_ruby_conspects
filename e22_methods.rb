...
# https://itproger.com/course/ruby/6   https://www.youtube.com/watch?v=i3LJ41hG1No
# Ruby School - Lesson 9   https://vimeo.com/100249761
...
# Оператор return возвращает из функции указанную после него строку кода и завершает работу метода
...
raznoe() # Выполняет все, что идет до и в return, не выпоняет все что после return.
...
# Переменные экземпляра класса работают(доступны) по всей программе(но ограниченны классами либо внутри либо вне) как внутри отделых методов так и вне их. Назначаются при помощи символа @
a = 1
@b = 1
def a_a
  a = 2
...
# Блок это совокупность фрагментов кода. Вы присваиваете имя блоку. Код в блоке всегда заключен в фигурные скобки ({}).
...
# some_meth{|e| block.call(e)} тоже самое что и some_meth(&block)
...
# lambda-функции(так же их называют lambda-выражения) - это указатель на функцию (без имени у самой функции)
