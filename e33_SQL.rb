...
# пример программы с запросоми CREATE, INSERT и SELECT и выводом элементов таблицы в консоль через методы вывода Руби:
...
# 1й Способ(в виде массива). Для того чтоб сделать вывод из базы данных - нужно создать селект-запрос. Это неудобно если мы выводим не все столбцы в запрсе, тогда индексы в переменой вводить будет неудобно, тк они станут другими.
db = SQLite3::Database.new 'barbershop.db'
db.execute 'SELECT * FROM Users' do |row| # в row присвается каждая строка данного запроса в виде массива значений строки
	puts row # выводим все элементы массива
	puts '------------'
	puts row[1] # тк row это массив то выводим только значение колонки с индексом 1
	puts '------------'
...
db.execute 'SELECT * FROM Users' do |row| # теперь наша переменная row клааса Hash а не Array и содержит ключами названия столбцов а значениями значения строки в этом столбце
  p row #=> {"id"=>1, "username"=>"Kroker", "phone"=>"657575", "datestamp"=>"2023-05-08T02:47", "barber"=>"Gus Fring", "color"=>"#25c137"}
	print row['username'] # теперь можно выводить по ключу и не будет путаницы, если в запросе не все столбцы
