...
# Содержание фаила 20230523033236_create_clients.rb (заполняется автоматически):
...
get '/barber/:id' do
  @barber = Barber.find(params[:id]) # find - метод актив рекорд(получает объект по праймари кей тут id). params[:id] - как и раньше принимает значение из адреса обработчика
