...
/pattern/im # может быть указано несколько опций
...
# За исключением управляющих символов (+ ? . * ^ $ ( ) [ ] { } | \) , все символы соответствуют самим себе. Вы можете экранировать управляющий символ, поставив перед ним обратную косую черту "\".
...
[...] # Соответсвует любому одиночному символу в скобках
[^...] # Соответсвует любому одиночному символу не указанному в квадратных в скобках
...
# Функциональные символы экранируем символом \
...
/([Rr]uby(,)?)+/ # соответсвует "Ruby, Ruby, ruby, ruby..." итд

# Back References(назад ссылки):
/([Rr])uby&\1ails/ # Соответсвует "Ruby&Rails" или "ruby&rails" (1 - берет 1й символ в выражении)
/(['''])(?:(?!\1).)*\1/ # Строка в одинарных или двойных кавычках. \1 соответсвует тому что соотв 1й группе. \2 соответсвует тому что соответсвует 2й группе итд
...
/^Ruby/ # Соответсвует "Ruby" в начале строки или внутренней линии(строки текста?)
/Ruby$/ # Соответсвует "Ruby" в конце строки или линии(line)
...
'Abc Probably probably'.gsub(/([pP])robably/, "\\1rolly") #=> "Abc Prolly prolly"   # phrase.gsub(/([pP])robably/,'\1rolly')
...
# Вернуть все буквы стоящие после буквы(слова?) заданной переменной letter в строке
