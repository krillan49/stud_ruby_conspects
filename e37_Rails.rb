...
# > rails new some_name   -  cоздаcт новое рейлс-приложение в выбранной папке(вручную ничего делать не нужно), где последнее слово на выбор это имя директории приложения(например blog). Устанавливаются все необходимое для работы приложения, гемы, инициализируется гит итд. В итоге появляется папка с выбранным именем, которая содержит все необходимые стартовые папки и фаилы(далее обозначим f - фаил, d - папка):
# f .git
...
#   (этот пункт делается в независимости от ошибок таймзон)
...
# В случае с Рэилс запрос браузера сначало попадает в Rails router(его конфигкрация находится в /config/routes.rb), а уже оттуда в соотв с машрутами передается в контроллер, контроллер делает запрос к модели, модель обращается в БД, передает данные в виде сущности контроллеру, он передает их в представление и затем возвращает html сгенерированной страницы в браузер.

#   Контроллер - выполняет обработку URL запросов от браузера. Контроллер понимает на какую страницу сейчас переходите и в зависимости от неё возвращает браузеру определенный HTML шаблон. Перед открытием шаблона контроллер может связаться с моделью и получить значения из БД, которые будут переданы позже в шаблон;
...
# Для создания новой страницы необходимо создать контроллер, а также прописать обработку всех URL адресов.
...
# Контроллер можно полностью создать в ручную, а можно при помощи специальных команд в терминале

# > rails generate controller home index  -  команда создания контроллера, где:
#     rails generate - команда создания чегото; controller - говорит о том что мы будем создавать контроллер;
...
# Маршруты хранятся в config/routes.rb там их тоже можно посмотреть и изменить.

# Пропишем маршрут(закрепим обработчики URLов за опред экшенами). Внесём изменения в файл /config/routes.rb
Rails.application.routes.draw do
  # Обычно home#index создаётся для главной страницы сайта, поэтому можно поменять корневой маршрут и прописать в нем вместо get 'home/index'
