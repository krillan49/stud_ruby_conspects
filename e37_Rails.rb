...
# controller - для генерации контроллеров(так же маршрутов и предсталений)
# model      - для генерации моделей и миграций
# migration  - генератор миграции, который создает миграцию без модели
# scaffold   - генератор одновременно модель, контроллер, экшены, маршруты, представления, тесты по REST resourses.
# ...
...
class Article < ApplicationRecord
  ...
  # + Теперь можно обращаться от любой статьи к коллекции принадлежащих ей комментов через Article.find(1).comments
end
...
puts
# Задача: чтобы мы могли просматривать статьи(/articles), но не могли их создавать(/articles/new), если неавторизированный пользователь:
...
class ArticlesController < ApplicationController
  ...
  # before_action - перед работой любого метода контроллера запускает фильр в аргументе :authenticate_user!.
...
class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters # метод фильтра с настройками разрешений
    devise_parameter_sanitizer.permit(:sign_up, keys: [:username]) # где :sign_up раздел(тут форма для регистрации нового пользователя), keys: [:username] - то что добавляем в этот раздел. Тоесть мы разрешаем добавление параметра :username для формы :sign_up. (или для занесения в БД из этой формы тк срабатывает перед экшенами девайс ??) Тоесть чтобы при регистрации теперь спрашивало username
...
# 5. Сделаем так, чтобы при создании комментария в поле автор указывался залогиненый юзер(articles/show.html.erb)
...
# 4. В контроллере article добавим(уберем доступ при ручном переходе по URL):
class ArticlesController < ApplicationController
  before_action :authenticate_user!, :only => [:new, :create, :edit, :update, :destroy]
  # before_action :owner?, only: %i[edit destroy]  # в примере предлагалось так вместо вызова метода в экшенах но чето не работает(мб потому что вызывается до присвоения сущности статьи в переменную ??)
  ...
  def owner?
    unless current_user == @article.user  # тут тоже без обращения к айди
...
# В файле rails_helper.rb есть закоментированный код:
Dir[Rails.root.join('spec', 'support', '**', '*.rb')].each { |f| require f }
# Если мы его раскомментим, то каждый раз вписывать наши новые вспомогательные файлы из support не надо будет. Они будут вписываться автоматически
...
# 3. Создадим тест для просмотра страницы создания статьи залогиненым пользователем /spec/features/user_creates_article_spec.rb:
require "rails_helper"
...
# Запустим миграции(тут с првильным синтаксисом, с гемами из гемфаила а не упрощенным)
# > bundle exec rake db:migrate
# Теперь мы можем создавать юзеров, редактировать, удалять и просматривать.
# Пример из users_controller.rb
def create
...
# Будет проверять в контроллере и затем выведет список ошибок
