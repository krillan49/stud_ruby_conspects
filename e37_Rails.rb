...
# Пропишем маршрут(закрепим обработчики юрллов за опред экшенами). Внесём изменения в файл /config/routes.rb
...
# params - объект который по умолчанию присутсвует(переходит при наследовании из ApplicationController) в контроллере(можно обратиться к нему из любого метода в контроллере), в нем хранятся все параметры которые передаются из браузера в приложение.
# https://api.rubyonrails.org/classes/ActionController/Parameters.html -  реализация params в ActionController(хэш хэшей)
...
params = ActionController::Parameters.new
params.permitted? # => false  # проверяем разрешен или нет только что созданный params, автоматически проверяется методами(сущности а не контроллера) изменяющими БД: save, create, updste, destroy
...
# render - когда пользователь обновляет страницу, он снова отправит предыдущий запрос POST. Это может привести к нежелательным результатам, таким как повторная покупка и другие. Соответвенно используем только в безопасных случаях, например когда не прошла валидация и данные не отправлены в БД

# redirect_to - когда пользователь обновляет страницу, он просто снова запросит ту же страницу. Это также известно как шаблон Post/Redirect/Get (PRG). После обработки запроса браузеру возвращается маленький пакет(тк не содержит представлений итд, а только управляющую команду), браузер автоматически исполняет команду и отправляет GET-запрос
...
# Все что нужно для удаления сущности: получить ее id, найти сущность по id, удалить
...
class ArticlesController < ApplicationController
  before_action :authenticate_user! # это значит что для доступа ко всем методам контроллера использовать фильтр(из гема devise) :authenticate_user!. Тоесть все методы будут недоступны неавторизованному юзеру и при запрсах будет перенаправлять на форму регистраци/авторизации devise
...
# 1. Добавим руками столбец username тк Devise по умолчанию(тут в таблице users модели User) не создает столбец username
# По идее можно просто убрать поле автора коммента и тк у нас есть доступ к email то мы можем на стороне сервера писать email, но логичнее использовать username для комментов в блоге
...
class AddUsername < ActiveRecord::Migration[7.0] # Теперь при помощи этой миграции мы можем добавлять в любые таблицы любые новые столбцы
  def change # метод change изначально не заполнен
    ...
    # Но так можно будет вставить в этот столбец неуникальный username, те может быть 2 одинаковых, что нам не подходит. Чтобы это исправить создадим на уровне БД уникальный индекс, который означает, что в этот столбец можно будет вставить только уникальные значения, если будем вставлять неуникальные, то ошибка будет уже на уровне БД
...
class ApplicationController < ActionController::Base
  ...
  before_action :configure_permitted_parameters, if: :devise_controller? # фильтр :configure_permitted_parameters будет выполнен только для devise контроллеров. Девайс контроллер это тот где у нас работает девайс, тут это Articles, тк там подключен девайс при помощи before_action :authenticate_user!
  ...
  def configure_permitted_parameters # метод фильтра с настройками разрешений
    devise_parameter_sanitizer.permit(:sign_up, keys: [:username]) # где :sign_up раздел(тут форма для регистрации нового пользователя), keys: [:username] - то что добавляем в этот раздел. Тоесть мы разрешаем добавление параметра :username для формы :sign_up. ?? Тоесть чтобы при регистрации теперь спрашивало username
...
# И в Лэйаут выведем имя вместо e-mail в приветсвии рядом с Sign Out в /app/views/layouts/application.html.erb
