...
p (1..10) === 5 #=> true
p ('a'..'j') === 'z' #=> false
puts "c lies in ('a'..'j')" if (('a'..'j') === 'c') #=> c lies in ('a'..'j')


puts
# Только для 2х элементов массива
arr=[1,7]
p Range.new(*arr).to_a #=> [1, 2, 3, 4, 5, 6, 7]
...
(1..4).inject{|sum, n| sum + n*n }    # => 30 (1**2 + 2**2 + 3**2 + 4**2)  # Sum of squares, without initial_operand.
(1..4).inject(2){|sum, n| sum + n*n } # => 32 (2 + 1**2 + 2**2 + 3**2 + 4**2)  # Sum of squares, with initial_operand.
...
(1..4).inject(:+)                # => 10           # Integer addition.
(1..4).inject(:*)                # => 24           # Integer multiplication.
('a'..'d').inject('', :+)        # => "abcd"       # Character range concatenation.
%w[foo bar baz].inject('', :+)   # => "foobarbaz"  # String array concatenation.

[100, 300, 2].reduce(:-) #=> -202

# Hash update.
h = [{foo: 0, bar: 1}, {baz: 2}, {bat: 3}].inject(:update) # => {:foo=>0, :bar=>1, :baz=>2, :bat=>3}
# Hash conversion to nested arrays.
h = {foo: 0, bar: 1}.inject([], :push) # => [[:foo, 0], [:bar, 1]]
