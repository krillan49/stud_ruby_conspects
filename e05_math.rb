...
puts (2+3)*5-6*2+7*3 # порядок действий идет как в математике. Скобки тоже как в математике
...
# Нахождение остатка от деления на отрицательное число. Тк остатое от деления равен велимое минус результат целочисленного деления умноженного на делитель то получаем:
...
p 1.6.to_r #=> (3602879701896397/2251799813685248)
p Rational('1.6') #=> (8/5)
p '1.6'.to_r #=> (8/5)
...
PositiveInfinity = +1.0/0.0 #=> Infinity
NegativeInfinity = -1.0/0.0 #=> -Infinity
CompleteInfinity = NegativeInfinity..PositiveInfinity #=> -Infinity..Infinity


puts
# Математические methods
num = -14.84
num.abs() #=> 14.84  #Модуль числа
num.round() #=> -14  #округление до целого
num.round(1) #=> -14.8  #округление до энного порядка
num.abs().round() #Можно задавать суффиксы сразу один за одним
num.positive? # Returns true if self is greater than 0, false otherwise.

(10.9999999999999).round(5) #=> 11.0

# Лежит ли число между числами(включительно):
a = 12
a.between?(10, 22)

5.round(-1) #=> 10  #округление до более высокого порядка
0.2.ceil # Округлкние вверх
2001.ceil(-2) #=> 2100
0.7.to_i # Округлкние вниз
0.7.floor
number.truncate(2) # Округление без изменений(обрезка до эн знаков после запятой/"целая" часть числа)
...
#`<=` - меньше или равно
p a <= b #=> true
#`<=>` - (только руби) космический корабль (spaceship operator). Возвращает 1 если первое больше 8 если равно и -1 если меньше
p a <=> b #=> -1
...
# Все делители числа кроме 1 и самого себя(хз насчет скорости)
arr=n.prime_division.map{|a| (1..a[1]).map{|i| a[0]**i}}
dels=(1..arr.size).map{|k| arr.combination(k).to_a}.flatten(1).map do |comb|
  if comb.size==1
    comb.flatten
  else
    comb[0].product(*comb[1..-1]).map{|a| a.inject(:*)}
  end
end.flatten.select{|e| e<n}

# восстанавливает исходное число из его сомножителей. Это именно метод класса, потому что выступает в роли «конструктора» целого числа.
require 'prime'
...
'6jnkjnkj'.to_i #=> 6  # Если цифра в строке 1я
...
# шестнадцатеричные числа могут также содержать символы a-f. Функция to_i умеет с ними обращаться.
"aB".to_i(base=16)   # 171
# если мы используем to_i без указания основания для строки с шестнадцатеричным числом, или с основанием, которое не предусматривает таких символов, тогда вернется 0
"aB".to_i            # 0
"9".to_i(base=8)     # 0
...
def convert(i, s, t)
  return t[0] if i=='0'
  arr=i.chars.map{|e| s.index(e)} # разбиваем на цифры и переводим цифры в десятичные по индексам строки нашей системы исчисления
  base1=s.size # мерность системы исчисления 1
  ...
  base2=t.size # мерность системы исчисления 2
  res=translator_from10_to_any(num, base2) # получаем n-сятки n-сотни n-тысячи нужной нам системы
  res.map{|i| t[i]}.join # переводим цифры из десятичных в необходимые нам по индексам
end
hex='0123456789abcdef'
allow='abcdefghijklmnopqrstuvwxyz'
p convert("hello", allow, hex)# '320048', '"hello" alpha_lower -> hex'
...
# Побитовый оператор OR  в Руби обозначается |
p 4 | 1 #=> 5
...
puts rand(4..10) # Выведет случайное число от 4 по 10
...
# тоже при помощи матриц( единицы добавляем если координаты зет нету )
require 'matrix'
