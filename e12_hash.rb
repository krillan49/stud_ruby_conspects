...
# в новых версиях языка руби хеш, который содержит не более 7 элементов реализован через массив. Снаружи мы это никак не определим, т.к. программист всегда использует API языка и не лезет во внутренности. Но если посмотреть исходный код языка и реализацию на языке “Си”, то эти подробности видны.
...
some = { # Конструкция ассоциативного массива
  ...
  # Символ => называется - Hash rocket(Устаревший символ)
	...
	'Ключ3(Key3)': 'Значение3(Value3)', # 'Ключ3(Key3)':  - это будет символ а не стринг(:'Ключ1(Key1)') Если ключ не задается кавычками и его синтаксис соответствует он автоматически становится symbol, соответсвенно numeric нельзя обозначить двоеточием как ключ.
  "RU" => 2.5,   # ключ String
...
student_marks.store('Math', 91) # метод тоже для добавления значений в хэш
...
# Инициализация хэша из массивов
a = [0, 1, 2, 3, 4]
...
# Одна из подобных задач - есть какое-то предложение, необходимо сосчитать частотность слов и вывести список. Например, слово “the” встречается 2 раза, слово “dog” 1 раз и так далее.
str = 'the quick brown fox jumps over the lazy dog'
arr = str.split(' ') # Разобьем ее на части. У нас получился массив слов.
# давайте обойдем этот массив и занесем каждое значение в хеш, где ключом будет слово, а значением - количество повторов этого слова
hh = {}
arr.each do |word|
...
hh = {:a =>'a', :b => 'b', :c => 'c'}
hh.keys #=> [:a, :b, :c] #hh.keys это массив ключей а значит можно применять к нему методы массивов напр hh.keys.each do |x|
...
# Оператор dig находит и возвращает объект во вложенных объектах, указанный ключом и/или иденксом.
h = {foo: {bar: [:a, :b, :c]}}
h.dig(:foo) # => {:bar=>[:a, :b, :c]}
h.dig(:foo, :bar) # => [:a, :b, :c]
h.dig(:foo, :bar, 2) # => :c
...
# Set представляет(реализует) собой коллекцию неупорядоченных неповторяющихся!!! значений (то есть без дубликатов)
...
def f(str) # наш метод, который принимает строку
...
# Записывается для хэша аналогично обычному массиву только задается 2 переменные: для ключа и для значения(1я переменная - ключ, 2я переменная - значение)
...
h2={"flour"=>100}
p h1.merge(h2){|k, ov, nv| ov-nv} #=> {"flour"=>100, "eggs"=>1, "sugar"=>100}
