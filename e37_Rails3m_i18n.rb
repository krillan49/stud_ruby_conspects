...
# так же помимо данного способа можно создавать отдельные виды для каждого языка(some.ru.html.erb) и возвращать их в соответсвии с тем какой язык выбрал пользователь(хранится например в таблице юзерс)
...
# Атрибуты(каталоги) для таблиц из БД(вылезают в ошибках валидации, формах и других местах например в своем выводе этих названий), в yml будут иметь структуру:
'activerecord:
  attributes:
    model_name:
...
module Blog2 # Blog2 - просто название проекта
  class Application < Rails::Application
    ...
    config.i18n.default_locale = :en # при ручном изменении параметра тут тоже можно переключать язык
  end
end
...
# 5. Метод и фильтр считывания локали из URL для экшенов(экшены теперь должны обрабатывать новые маршруты c локалью языка), для дальнейшего подключения нужного языка в возвращаемых этим экшеном представлениях. Добавляются в ApplicationController, а остальные наследуют
class ApplicationController < ActionController::Base
  ...
  # Вариант 2 - с подметодом проверки локали(те только те локали что мы прописали в конфиге, без левых)
  ...
  # Для обоих вариантов. Для того чтобы локаль хранилась в каждой ссылке и сохранялась между запросами, тоесть при переходе на другую страницу сохранялся выбранный язык(!!! работает только при помощи переходов ссылок-хэлперов link_to !!!)
...
# (!!! Есть проблемы с запоминанием локали для статических страниц, например страница для user_content)
