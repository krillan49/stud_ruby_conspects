...
class ApplicationController < ActionController::Base
  ...
  # (!!! работает только при помощи переходов ссылок-хэлперов link_to c хэлпером для URL !!!)

  # (!!! Есть проблемы с запоминанием локали для статических страниц, при переходе на них по хардкод ссылке, например:
  # <a href="/protected/<%= user.id %>"><%= user.username %></a>)
  # Причем если интерполировать локаль то: No route matches [GET] "/en/en/protected/1", значит маршрут и так норм. Но params[:locale] ничего не содержит в экшене который обрабатывает запрос из ссылки
  # Может обрабатывается правильная ссылка с локалью, но информация в params приходит из самой ссылки
end
