...
# Пример содержания фаила Gemfile(в данном случае для приложения на Синатре)
source "https://rubygems.org"

gem "sinatra"
gem "sinatra-contrib"  # этот гем содержит sinatra-reloader(на самом деле это тоже разработческий геи и соотв можно поместить в group :development и его тоже)
gem "sqlite3"
gem "activerecord"
gem "sinatra-activerecord"
gem "rake"

group :development do # данный синтаксис означает, что когда мы зальем приложение на хостинг, будет понятно что этот гем нужен исключительно для разработки, а не для работы приложения
  gem "tux"
end

# bundle install  - установка(из активной директории в которой лежит Gemfile)

# После установки в фаиле Gemfile.lock (Нужно создать заранее??) появятся записи для того чтоб в нашем приложении не было конфликта различных версий гемов(в этом фаиле были залочены определенные версии гемов)
