...
# Модули позволяют помещать различные методы в отдельный фаил, чтоб не захламлять ими основной код. В разные модули можно поместить методы с одинаковыми именами и тогда они никак не будут конфликтовать в основном коде.
...
p х = Bmodule.somemeth(Bmodule::PI/4) #=> 11  # Вызов метода модуля с помещением константы в параметр и итогового значения в переменную
...
puts
puts '                                  Mixins/Примеси(Подключение 2+ модуля в класс)'
