...
# 6. На странице всех вопросов, через паршал _question.html.erb рендерятся теги для конкретного вопроса в паршале tags/_tag.html.erb, в ссылке которая активирует доп функционал поиска других вопросов с тем же тегом, при нажатии на который должна перезагружаться вся страница с соответсвующими вопросами. Но при нажатии на ссылку ничего не происходит, тк она находится в турбо-фрэйме и соответсвенно запрос идет в формате турбофрэйма, а наш функционал тут должен обновлять не отдельный фрэйм а всю страницу, тоесть нам нужен запрос в html.
