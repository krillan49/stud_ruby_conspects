class ApplicationPolicy
  ...
  def initialize(user, record)
    @user = user      # текущий пользователь передается по умолчанию от метода current_user при инициализации нового объекта ApplicationPolicy, который создается когда мы вызываем метод authorize в контроллере
