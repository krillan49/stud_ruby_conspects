...
# > rails new blog   -  cоздаcт новое рейлс-приложение в выбранной папке(вручную ничего делать не нужно), где: blog это имя директории приложения(название без нижних подчеркиваний в камэлкейс, но можно с большой буквы). Устанавливаются все необходимое для работы приложения, гемы, инициализируется гит итд.
...
# Но будет еще одна ошибка тк пытается использовать фаил bin\rails, но на виндоус это не работает. Нужно сделать изменения в фаиле Procfile.dev(он запускает: 1 Сервер Рэилс, 2 Компиляцию JS, 3 Компиляция для CSS).
# в нем неняем первую строку
