...
# PUT, DELETE отправляются через POST, но с добавлением скрытой переменной(поля).
...
# Гет/пост - условное деление (конвенция), когда люди договорились, что:
# гет - это "получение информации", хотя в гет запросе можно передать инофрмацию в адресной строке браузера, например, поисковые запросы в гугле.
# пост - "передача информации" в виде заполненных форм
