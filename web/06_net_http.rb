...
# Для клиента сервер выступает единым источником, но при этом он может быть распределенным или состоять из кучи микросервисов.
...
# Можно дополнительно передать данные(оч небольшие), но эти данные будут видны в строке запрса(после знака ?), например:
#     /cart?product_1=4,product_2=7,product_3=4,
# а  адресе запроса после ? будут видны параметры, которые разделяются знаком &
...
# Гет/пост итд - это условное деление (конвенция), о которой просто договорились. Тоесть ничего не мешает создавать ресурс, например через GET
...
# Статус код(выше 200 OK) - сообщает о том как был обработан запрос. Существует 5 групп статус кодов:
...
# Корректно реализованные методы GET, PUT, DELETE - демпотентны; но не метод POST, тк во многих случаях создаст новую сущность идентичную предыдущей
...
def is_wrong_password?(password) # оформляем проверку доступа в виде функции
  ...
  response.include?('Wrong username or password, please try again') # проверку просто возвращаем
end
