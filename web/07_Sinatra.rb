...
# erb (эмбэдед руби - встроенный руби) - это HTML фаил позволяющий делать вставки на языке Руби
...
post '/' do # в пути(тут '/') пишется относительный адрес из затрибута action той формы, из которой мы хотим обрабатывать запрос
  ...
  # При помощи метода params мы можем присвоить введенные пользователем в поле данные (значение хэша params) в переменную.
...
get '/details/:post_id' do # добавляем в адрес :id(название любое) теперь Синатра будет знать что это идентификатор, соответсвенно данный обработчик становится универсальным для любых(не только цифры) значений :id. Теперь при переходе на адрес '/details/некое_значение' (не важно на адрес этот перешли ссылкой или мы просто пишем значение в адресной строке наугад) будет задействован этот обработчик
