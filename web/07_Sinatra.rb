...
post '/' do # в пути(тут '/') пишется относительный адрес из затрибута action той формы, из которой мы хотим обрабатывать запрос
  ...
  # При помощи метода params мы можем присвоить введенные в пользователем в поле данные в переменную(значение хэша params).
