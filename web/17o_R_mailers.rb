...
# 2. Напишем метод генерации токена. Можно было бы сделать и в модели user.rb, но она и так слишком разрослась, потому создадим новый консерн моделей concerns/recoverable.rb и создадим в нем собственно метод set_password_reset_token (код там) который будет создавать токен в экшене create, чтобы отправить его в ссылке в письме
