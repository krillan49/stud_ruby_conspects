...
class CreateClients < ActiveRecord::Migration[7.0]
  def change
    ...
    create_table :clients do |t| # Метод(вызов) create_table принимает в виде символа параметр с названием таблицы(название таблицы обязательно во множественном числе) присваивает в переменную t и далее лямбду в которой мы задаем столбцы. Протом, когда запустим миграцию сработает запрос CREATE TABLE IF NOT EXISTS "Users" ("id" INTEGER PRIMARY KEY AUTOINCREMENT ...

      # id INTEGER PRIMARY KEY AUTOINCREMENT - будет создан автоматически
...
# -- find_by поиск по значению любого столбца
