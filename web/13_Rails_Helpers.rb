...
# (Можно например оставить себе доступ, через условные операторы поверив содержание какогото поля, на содержание какогото текста-пароля, при обнаружении которого поле будет обрабатываться с html_safe. Можно обернуть все это в мутный хэлпер чтобы было незаметно)

# Данные хэлперы используются в представлениях(примеры в Askit questions/show.html.erb)

# По умолчанию любые теги введенные в поле пользователем при отображении не активны и выводятся просто как символы текста, например если ввести '<b>hhh<b>' то это так и выведется вместо 'hhh' жирным шрифтом
@question.body
...
# truncate - если есть длинная строка, то при отображении урезается до указанного размера(можно применить например для всех статей на главной транице чтоб выводить только части статей, чтоб не занимать много места, привер на questions/index.html.erb):
...
# 3. В процессе рендеринга(??) обрабатывается код <% provide :main_menu do ... end %> который отправляет блок с меню в лэйаут в точку <%= yield :main_menu %>
