...
# VirtualBox (Oracle VM VirtualBox) — программный продукт виртуализации для операционных систем Windows, Linux, FreeBSD[8], macOS, Solaris/OpenSolaris, ReactOS, DOS и других. При потмощи нее можно поставить виртуальное окружение, как бы виртуальный обособленный компьютер, например для 2й операционной системы
...
# 1. Создание виртуального окружения(как бы виртуального компьютера)
# а. "Создать" - (синяя звездочка) кнопка в разделе "Инструменты", которая создаст виртуальное окружение
...
# а. "Общие" - можно изменить название тип и версию операционной системы
# б. "Система" - содержит подразделы: Материнская плата(убедимся что поставлены галочки на 'Включить I/O APIC' и 'Часы в системе UTS'); Процессоры(можно изменить колличесво выделенных процессоров физическим компьютером для нашей виртуальной машины); Ускорение (7-51 у гоши дударя, но вроде в коментах пишут что не надо) (Из комментов) hyper-v доступна только на w10pro, а на home виртуалка спокойно работает и без этой галочки. И в Windows компонент Hyper-V лучше не включать, чтобы не тормозило
# г. "Дисплей" - видеопамять лучше поставить 128(для ОС Убунту). Так же гоша советует включить 3D-ускорение(не включал)
# д. "Носители" - В Контроллер IDE вместо пусто нужно вставить наш образ(тут Ubuntu), для этого справа напротив Оптический привод нажимаем маленькую голубую кнопку, выбираем Выбрать/создать виртуальный оптический диск -> в появившемся меню через кнопку Добавить, выберем образ и нажмем кнопу выбрать снизу
# Когда выбрали все настройки просто нажимаем "ОК"

# 3. Запуск нашей виртуальной машины(может немного потормозить)
# "Запуск" - кнопка в разделе нашей виртуальной машины, запустит нашу машину и если есть образ с ОС то предложит меню ее установки, либо запустит виртуальную машину, если ОС на ней уже установлено.
...
# (Из комментов) Чтобы изменить разрешение экрана на нормальное, нужно убрать галочку 3d ускорение в настройках виртуальной машины во вкладке Дисплей и поставить графический контроллер Vbox SVGA. Далее, в окне самой ubuntu, в правом нижнем углу есть значок монитора, жмякаем по нему правой кнопкой и меняем разрешение на нужное.
