...
# Ruby
# SQL-СУБД(SQLite3, PostgreSQL итд)
# Node.js
# Yarn
...
# https://rubyinstaller.org/downloads/   -  Устанавливаем Руби, версия с WITH DEVKIT, выделенная жирным.
...
# желательно поставить галочку на "Хотите ли вы доставить дополнительные компоненты для компилирования библиотек"(инструменты для компиляции библиотек, тк некоторые библиотеки нужно компилировать). Если галка не была поставлена, то чтобы узнать, нужно установить дополнительно - нужно воспользоваться руководством тут https://github.com/nodejs/node-gyp
# > node --version
...
# 2. Установка Руби через RVM для *nix-систем(Unix-подобная операционная система, например Линукс и МакОС):
...
# > rails new blog   -  cоздаcт новое рейлс-приложение в выбранной папке, где:
...
# package.json (содержит библиотеки для фронтэнда) добавим строки:
"@rails/ujs": "^6.0.0"
...
# app/javascript/application.js     - вносим изменения(они там и описаны)
...
# 1. На Виндоус(64) в Рэилс 7 с Руби версий 3.1(и выше ??), тк эти версии сообщают новую версию платформы, которая не распознвается в бандлере. По умодлчанию будет выдавать ошибку таймзон(tzinfo-data is not present. Please add gem 'tzinfo-data' to your Gemfile and run bundle install (TZInfo::DataSourceNotFound)), поэтому нужно изменить в Gemfile строку:
...
puts
# Запуск приложения с вебпакером(или другими инструментами для js ассетов).
# Не подойдет стандартный вариает, тк надо запустить вебпакер и перекомпилировать наши scss каждый раз когда они меняются

# На никс-системах мы просто вводим в терминал bin\dev и сервер бы запустился(берет инструции из этого фаила bin\dev сам)
...
# 2. На Винде при старте проекта или при различных манипуляциях, например при обгновленни версии в фаилы bin/rails, bin/rake и bin/setup в первую заккоментированную строку в конце может добавляться расширение .exe, его нужно удалить, тк могут возникнуть проблемы при развертывании приложения в продакшене
