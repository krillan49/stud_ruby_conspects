...
# Gemfile для приложений Синатры с ActiveRecord(или для любых ??)
...
class CreateClients < ActiveRecord::Migration[7.0]
  def change
    ...
    create_table :clients do |t| # оператор вызова метода create_table принимает в виде символа параметр с названием таблицы(обязательно во множественном числе) и блок в котором мы задаем столбцы, присваивая в переменную t имя таблицы. При запуске миграции сработает запрос CREATE TABLE IF NOT EXISTS "Users" ...
