...
# 3. Создадим модель - класс который будет, взаимодействовать с одноименной(тут clients) таблицей и представлять нашу сущность(entity). Экземпляры модели это описание строк из этой таблицы на Руби, создаются либо методами класса модели в виде колекций, либо через new(примеры сущностей: клиент, парикмахер, статья, комментарий). Класс сущности называется в единственном числе, тк объекты класса сущности будут содержать иформацию об одном экземпляре сущности(тут о конкретном клиенте) те о том что будет находится в одной строке БД.
...
class CreateClients < ActiveRecord::Migration[7.0]
  def change
    ...
    create_table :clients do |t| # оператор вызова метода create_table принимает в виде символа параметр с названием таблицы(обязательно во множественном числе) и принимает блок в котором мы задаем столбцы, присваивая в переменную t имя таблицы. При запуске миграции сработает запрос CREATE TABLE IF NOT EXISTS "Users" ...
...
# (создают в методе класса экземпляры строк таблиц по запросу ??)
