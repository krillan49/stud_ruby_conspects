...
# ActiveRecord - это специальный гем для Ruby, который делает работу с БД более удобной и быстрой, он позволяет работать с БД как с обычными классами Руби, те вместо написания SQL-запросов мы можем писать рубикод-запросы(хотя и обычные SQL-запросы писать тоже можно)
...
# 3. Создадим модель - класс который будет, взаимодействовать с одноименной таблицей и представлять нашу сущность(entity), экземпляры которой это описание строк в этой таблице. Модель/сущность описывает наши события/строки в таблице(тут clients) - то с чем мы работаем(примеры сущностей: клиент, парикмахер, пост в блоге, комментарий итд). Класс сущности называется в единственном числе, тк объекты класса сущности будут содержать иформацию об одном экземпляре сущности(тут о конкретном клиенте) те о том что будет находится в одной строке БД.
...
class CreateClients < ActiveRecord::Migration[7.0]
  def change
    ...
    create_table :clients do |t| # Метод(вызов) create_table принимает в виде символа параметр с названием таблицы(название таблицы обязательно во множественном числе) присваивает в переменную t и далее принимает блок в которой мы задаем столбцы. Протом, когда запустим миграцию сработает запрос CREATE TABLE IF NOT EXISTS "Users" ("id" INTEGER PRIMARY KEY AUTOI...
...
class CreateBarbers < ActiveRecord::Migration[7.0]
  def change
    ...
    Barber.create :name => 'Jessie Pinkman'  #=> Создаем экземпляр класса сущности в таблице с данными задаваемыми параметрами метода create, в виде хэша где ключ это название столбца а значение это значение в строке этого столбца
...
# 2. Some.new/some.sawe - new создает объект только в памяти и чтобы его сохранить в БД нужно выполнить еще метод save.
...
b.save #=> по умолчанию проводит валидацию и вносит(создает или заменяет) сущность в таблицу (INSERT INTO "barbers" ("name", "created_at", "updated_at") VALUES (?, ?, ?)  [["name", "Petya"], ["created_at", "2023-05-26 03:49:05.241644"], ["updated_at", "2023-05-26 03:49:05.241644"]])
...
Customer.find(10) #=> #<Customer id: 10, first_name: "Ryan">  # SELECT * FROM customers WHERE (customers.id = 10) LIMIT 1  # Поиск по ключу  # вызовет ошибку если запись не будет найдена
