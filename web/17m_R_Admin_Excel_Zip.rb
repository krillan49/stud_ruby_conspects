...
# (На примере AskIt) Создадим функционал, который позволит администратору сайта выгружать(скачивать) информацию о пользователях в формате Excel, а именно xlsx, разбитую по отдельным Excel фаилам для каждого пользователя и чтобы все эти фаилы вместе помещались в один архив ZIP, чтобы можно было скачать этот архив и работать с фаилами в нем.
  ...
  class UsersController < ApplicationController
    def index
      respond_to do |format|
        ...
        format.html do # формат html, пишем в блоке тоже что пишем в контроллере по умолчанию, тк это формат по умолчанию
          @pagy, @users = pagy User.order(created_at: :desc)
    ...
    def respond_with_zipped_users # в консерн можно не переносить, тк использовать будем только тут
      ...
      compressed_filestream = Zip::OutputStream.write_buffer do |zos|
        ...
        User.order(created_at: :desc).each do |user| # создадим отдельный Excel фаил для каждого пользователя из запроса
          ...
          zos.print render_to_string(
            ...
            # render_to_string(...) - метод который генерирует Excel-фаил, данный рендер будет происходить просто в памяти и делать некую строку
...
s = wb.styles # создаем стили для визуала яцеек таблицы Excel (не обязательно)
...
wb.add_worksheet(name: 'User') do |sheet| # добавляем новый лист с именем 'User'
  ...
  # style: [nil, highlight_cell] - применяем стиль, в массиве к 1й ячейке не применяем стили потому nil, а во второй применяем стили созданные за переменной highlight_cell
...
puts
puts '                         Загрузка данных из xlsx из архива в БД. Сервисные классы'

# (!!! Не записывает в БД, незаархивированный xlsx. Потом если понадобиться можно улучшить, чтоб применял и незазипованный)

# Сделаем так чтобы админ мог загрузить архив с Excel фаилами и внести данные их них в БД, тоесть создать новых юзеров в БД, на основании строк из таблицы Excel. Это хороший вариант наполнения БД админом если нужно ввести в систему сразу множества новых пользователей.

# Нам нужно загрузить фаил zip в приложение, там открыть фаил zip, вытащить из него фаилы Excel и обработать каждый из них и на основе них создать пользователей в БД, так же проделать и валидации.
...
# 1. Добавим форму для загрузки фаила(zip архива) в представление admin/users/index.html.erb
  ...
  class UsersController < ApplicationController
    ...
    def create
      if params[:archive].present?  # проверяем был ли передан архив в форме
        UserBulkService.call params[:archive] # передаем zip архив переданный из формы на обработку в сервисный класс в метод self.call
        flash[:success] = 'Users imported!'
      end
      # потом при желании можно модифицировать, например если архив не передан, то надо создать только одного пользователя на основе переданных параметров (стандартно). Можно сделать одельную страницу или форму.
...
# г) Создадим наш сервисный класс для опбаботки архива services/user_bulk_service.rb и напишем в нем методы обработки и записи в БД
