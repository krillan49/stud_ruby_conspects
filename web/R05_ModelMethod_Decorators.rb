...
# Декораторы нужны для того, чтобы добавлять к нашим объектам дополнительные методы, которые в себя включают логику с отображением именно этого объекта. Они живут не в глобальном пространстве имен, а только для тех конкрктных объектов(а не всей модели), для которых мы их назначим
...
# 2. Задекорируем текущего юзера(сделаем юзера способным вызывать методы декоратора) прямо в хэлпере в application_controller.rb(или в соотв консерне authentication.rb) те в том месте где его находим/определяем
def current_user
