...
#     2-1. Добавим строку(если ее нет) в фаил config/environments/development.rb:
...
#     2-3. Добавить(если нет) в Лэйаут нашего приложения app/views/layouts/application.html.erb. Нужно для работы технологии флеш-сообщений:
# <p class="notice"><%= notice %></p>
# <p class="alert"><%= alert %></p>

#     2-4. Для кастомизации форм - можно сделать свои шаблоны для форм(логина, пароля итд), либо отредактировать те которые выдает гем. Но по умолчанию в каталоге views раздела и представлений для devise нет, поэтому, если нужно, то можно сгенерировать набор views что использует devise по умолчанию:
# > rails g devise:views
# Команда автоматически сгенерирует множество директорий и фаилов в views в новой подпапке devise, например представление с формой регистрации. Теперь мы можем эти представления изменять
...
# /app/views/devise/registrations/new.html.erb - добавим для sign up поля ввода для username
# В лэйаут выведем имя вместо e-mail в приветсвии рядом с Sign Out в /app/views/layouts/application.html.erb
# Изменим форму комментов в articles/show.html.erb чтобы при создании комментария в поле автор указывался залогиненый юзер
...
class ApplicationController < ActionController::Base
  ...
  def configure_permitted_parameters # метод фильтра с настройками разрешений
    ...
    # Тоесть мы разрешаем добавление параметра :username, что добавмим в из форму :sign_up.
...
# 3. Рекомендуется, но не обязательно: скопируйте (или переместите) представления из devise/sessionsв users/sessions. Rails продолжит использовать представления devise/sessions из-за наследования, если вы пропустите этот шаг, но наличие представлений, соответствующих контроллеру(ам), обеспечивает согласованность.
