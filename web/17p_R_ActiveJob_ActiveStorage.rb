...
# ActiveJob - (фича Рэилс) это функционал, который позволяет отправлять наши задачи предназначенные для выполнения в фоновом режиме различным адапретам(kafka, sidekiq), которые и выполняют эти задачи в фоновом режиме, те управляют очередями, задачами итд

# Тоесть пользователь не ждет пока зада будет выполнена, а может дпльше чето делать в приложении и ничего не подвиснет, тк задача будет выполнена в бэкграунде.

# Но по умолчанию это не работает, тк обработка в бэуграунде изначально не настроена. Для того чтобы ее настроить нужно специальное решение, которое будет правильным образом выполнять обработку, ставить задачи в очередь, при необходимости выполнять задачи снова, если возникла ошибка итд
...
# Для винды есть альтернативное решение, которое полностью повторяет API Редиса: Memurai https://www.memurai.com/
...
# Запускать sidekiq будет в отдельном окне командной строки в директории проекта, тк sidekiq запускает как бы свой сервер и идет свой отладочный вывод
...
class AdminConstraint
  def matches?(request) # обязательно должен быть этот метод
    ...
    # Либо пускаем этого пользователя по этому маршруту или нет, в зависимости от того что вернет метод.
  end
end
  ...
  class UsersController < BaseController
    ...
    def create
      if params[:archive].present?
        # UserBulkService.call params[:archive] - раньше вызывали сервисный объект services/user_bulk_service.rb, теперь будем вызывать задачу:
...
# Так как мы не сможем нормально воспользоваться загруженным архивом из params[:archive] для передачи его в метод perform_later джоба user_bulk_import_job.rb, тк это временный архив и может уже удалиться до момента передачи, то нам нужно сделать этот архив не временным, тоесть нам нужно его гдето сохранить не во временной папке, а гдето в постоянном хранилище.
# Можно использовать облачные(например Амазон) и локальные хранилища. Мы могли бы написать собственный скрипт, который берет этот архив и помещает в другую папку, но проще задействовать функционал - ActiveStorage.
...
module Admin
  class UsersController < BaseController
    ...
    def create
      if params[:archive].present?
        UserBulkImportJob.perform_later create_blob, current_user # в параметры добавляем наш метод(айди загруженного фаила) и текущего юзера, чтобы пнапример на его имэйл послать письмо с подтверждением выполнения задачи итд
        flash[:success] = t '.success'
      end
      redirect_to admin_users_path
    end
    ...
    def create_blob
      ...
      # ActiveStorage::Blob.create_and_upload! - создает и закружает фаил из временного
      ...
      file.close
      result.key # key - вернет уникальный идентификатор загруженного фаила в ActiveStorage, он будет храниться в БД, в одной из таблиц ActiveStorage
    end
...
# 2. Модифицируем сервисный объект services/user_bulk_service.rb (Admin_Exel_Zip), тк будем использовать его в джобе user_bulk_import_job.rb
# app/services/user_bulk_import_service.rb - переназовем его более подходяще тк он занимается импортом. (код там)
...
# б. в представлениях админа создадим новую директорию admin/user_mailer и представления для писем в ней для каждого формата писбма и языка отдельное, с базовыми названиями, соотв названиям методов мэйлера bulk_export_done, bulk_import_fail
