...
# 1. find - команда выполняет поиск файлов и папок по их названию, типу, размеру итд и выводит их названия в консоль. Команда ищет в указанной дирректории и всех вложенных в указанную директориях
...
# $ ls | grep t               - выведет все имена фаилов в которых есть буква "t"(без кавычек), тоесть ищет именно в выводе консоли, а не внутри фаилов как обычно
...
puts
puts '                            Работа с памятью, сжатие файлов и редакторы текста'
...
# Архивация файлов может быть выполнена при помощи команды tar. Эта команда не сжимает файлы, а лишь создает архив (?? создает архив только из папки с фаилами, но не из отдельных фаилов ?? потом проверить).
# $ tar cf archive.tar video                  - будет создан архив «archive.tar» из папки video и ее содержимого


# Если нужно не только создать архив, но и сжать файлы, то можно воспользоваться одним из доступных методов компресии: zip, gzip, bzip2, xz. Все они сжимают файлы. Разница в методе, скорости и эффективности сжатия(компрессии).

# Если архив нужно отправить Windows пользователю, то лучше использовать стандартный zip.



# В Linux в зависимости от версии могут присутствовать разные встроенные текстовые редакторы. Наиболее популярными являются: nano, vim, gedit, pico
