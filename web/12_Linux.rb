...
# 1. touch - команда позволяет создать один или несколько файлов в определенной директории. Не может создавать папки
...
# 2. nano - команда позволяет открыть фаил для редактирования, но если фаил не существует, то он будет создан и открыт. В окне редактирования можно изменять фаил, снизу прописаны дополнительные команды, где символ "^" обозначет ctrl
# $ nano index.html                      - Будет открыт файл index.html
# $ nano www/index.html                  - Будет открыт файл index.html в папке www
...
# 6. rm - команда удаляет один или несколько файлов. Для удаления папок нужно дописать опцию -rf
...
# При создании фаилов или папок от имени администратора(попросит пароль после нажатия Enter), они будут иметь закрытые права доступа и пометку на значке фаила. Это означает, что их редактирование и удаление может быть выполнено только супер пользователем.
...
# # su krillan                   - (небезопасно) переход из режима администратора обратно в режим пользователя
...
# 1я цифра - (тут 7) права доступа для owner/владельца фаила (тот кто его создал) (u)
# 2я цифра - (тут 6) права доступа для group/группы                               (g)
# 3я цифра - (тут 5) права доступа для всех остальных                             (o)
...
# Где 1й символ(- или d) это обозначение фаила или папки соответсвенно, далее права доступа для всех 3х типов(rw- r-- r--), далее хз что за цифра, дплее владелец(root) и группа(root), далее вес фаила, дата и его название
...
# Теперь пользователи из группы krillan могут редаксировать фаил без прав суперпользователя
...
# $ apt-cache policy vim         - выдаст инфу о приложении есть ли на компе и если нет где можно скачать и предпочтит версию
...
# 5. apt-cache search - команда которая ищет пакет(приложкение) в стандартном репозитории. Может искать по неполному названию и прелагать варианты в зависимости соотв введенных символов.
...
# 1. find - команда выполняет поиск файлов и папок по их названию, типу, размеру итд и выводит их названия в консоль
...
# -iname - флаг для поиска по названию папки или фаила без учета регистра   [-name/-iname] "*.txt"
# -perm - флаг для поиска по коду доступа, далее нужно указать код (? добавив 0, хз зачем и без него работает) например 0764
# -size - флаг для поиска по размеру фаила, перед числом размера можно указать +(больше чем) или -(меньше чем), а после единицу измерения, например k(килобайт) или M(мегабайт) -size [+/-][число][k/M]
...
# $ find . -type f -perm 0664                 - находит в текущей директории все фаилы с доступом 664
# $ find . -type f -size +1M                  - находит фаилы больше 1 мегабайта
# $ find . -type f -size -10k                 - находит фаилы меньше 10 килобайт
...
# 2. grep - команда для поиска информации внутри файлов и отображает строки, где было найдено совпадение. Искомый текст выведет выделенным красным, при поиске в нескольких фаилах выведет названия фаилов перед найденным в них текстом
...
# И с I/O (потоками ввода/вывода) в линуксе выполнить цепочку команд проще. Передаем результат, как аргумент с помощью синтаксиса "команда которая выдат что-то в терминал" | "команда, которой в качестве аргументов передаётся прошлый результат, те это чтото"

# $ ls | grep "main"                  - выведет все строки из всех файлов что вывела команда ls, которые содержат "main"
