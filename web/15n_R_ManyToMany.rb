...
# Так же добавим индекс(можно было в миграции талицы тегов) в в таблицу тегов для проверки уникальности названия тега
...
# Различия между scope и методом класса: scope всегда возвращает relation, а обычный метод класса может вернуть nil, поэтому для метода класса нужны доп проверки или использовать &. scope удобный инструмент, но реализация скрыта что может привести к проблемам при использовании со сложной логикой. Поэтому, при простой логике - scope, при сложной - метод класса. Хотя scope тоже может содержать сложную логику, тк может быть расширен модулями scope.extended
...
# TomSelect - по умолчанию имеет поддержку бутстрап 5
...
# Создадим подпаку script в директории app/javascript и в app/javascript/script создадим фвил select.js в котором собственно и будет код скрипта для нашего селектора с TomSelect (можно было бы и прямо в application.js писать скрипт, но тогда там будет каша), не забываем подключить фаил select.js в application.js
...
# Но для него в контроллер вопросов придется добавить запрос на все теги читобы заполнить селектор
def index
  # ...
...
# Далее создаем дирректорию app/blueprints (название любое ??) и фаил tag_blueprint.rb в ней с классом содержащим описание того как нужно сериализовать наши теги в json
