...
class CreateQuestionTags < ActiveRecord::Migration[7.0]
  def change
    ...
    # Добавим вркчную только этот дополнительный индекс, чтобы в таблице не появилось 2х записей с одинаковой комбинацией :question_id и :tag_id, тк нет смысла для одного вопроса добалять одинаковые теги
...
# 6. Сделаем функционал поиска вопросов по тегу в tags/_tag.html.erb. Можно было бы это делать в контроллере, но лучше сделаем основной функционал в модели:
...
def index # GET '/questions' или GET '/questions?tag_ids=1'
  ...
  @pagy, @questions = pagy Question.all_by_tags(@tags) # выбираем только те вопросы, которые имеют тот тег, айди которого мы получили из params[:tag_ids], если же айди нет то выбираем все вопросы
  # all_by_tags(@tags) - статический метод модели, который и делает вышеописанное, передаем в него тег из запроса
...
class Question < ApplicationRecord
  ...
  # scope :all_by_tags - это в Рэилс по сути тоже самое что и def self.all_by_tags, тоесть метод класса модели, но так визуально мы понимаем, что это метод, при помощи которого мы можем выбирать определенные записи из БД по неким критериям
...
# Различия между scope и методом класса: scope всегда возвращает relation, а класс может вернуть nil, поэтому для метода класса нужны доп проверки или использовать &. scope удобный, наглядный  инструмент, но реализация скрыта под капотом что может привести к неожиданным проблемам при использовании со сложной логикой. Поэтому, при простой логике - scope, при сложной - метод класса. Хотя  scope тоже может содержать сложную логику, тк может быть расширяемым модулями scope.extended
...
# TomSelect - решение для удобного селектора со стилями и JS. Канал SupeRails делал ролик по этой библиотеке с реализацией на ванильном JS и Stimulus JS. Ещё подробнее инструкция - https://blog.corsego.com/select-or-create-with-tom-select. Эта реализация интересна ещё и тем что можно автоматически создавать тэги прямо в форме создания объекта. Есть алгоритм использования этой библиотеки с async/await.



# (Альтернатива serialize: jBuilder - урок 16)

# serialize - генерирует объект json

# questions/index.html.erb - добавим секцию с формой для поиска фопросов по тегам, при помощи которой будем отображать на главной те вопросы у которых есть выбранные в селекторе(похож на селектор сделвнный ранее) формы теги

# Так же добавим в контроллер вопросов запрос на все теги для заполнения селектора
def index
  # ...
  @tags = Tag.all if !@tags # добавляем список всех тегов для селектора тегов, если это запрос из селектора
end

# Но использование каждый раз Tag.all для селектора слишком затратно, тк отобразит абсолютно все теги, поэтому удалим эту строку с запросом и вместо этого реализуем асинхронный поиск при помощи ajax и пространства имен которое назовем api:

# Создадим новое простанство имен namespace :api и в нем маршрут для тегов
...
module Api
  class TagsController < ApplicationController
    def index
      ...
      # render json:  -  возврвщаем объект json
    end
  end
end
