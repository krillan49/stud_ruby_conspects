...
class QuestionsController < ApplicationController
  ...
  def set_question! # метод который будет запускаться при помощи before_action, называем с восклиц знаком в конце, тк он может вернуть ошибку если не существует такой вопрос
    ...
    # find для этой цели лучше чем find_by, тк если пользователь введет адрес несуществующего вопроса, например /questions/266666, то возникнет ошибка ActiveRecord::RecordNotFound в контроллере(в этом методе), а не ошибка в виде, соотв проще будет эту ошибку обработать
  end
end
