...
# Добавим в questions/index.html.erb специальный хэлпер kaminari - paginate для того чтобы переключаться по страницам. Этот метод базовый, есть и другие с различными дополнитеотными параметрами
...
# 0. Удалим папку каминари с паршалами и фаил каминари из конфига
...
def index
  ...
  # @pagy - специальный объект с помощью которого будет отрисовываться навигация (кнопки переключения страниц)
...
# 6. Тк в pagy нет по умолчанию скрытия меню пагинации если всего одна страница, то сделаем это кастомно на примере questions/index.html.erb так же вынесем это в кастомный хэлпер pagination для удобства
module ApplicationHelper
  include Pagy::Frontend
