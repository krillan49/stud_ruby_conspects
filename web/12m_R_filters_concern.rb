...
# before_action  -  это действие(метод), которое выполняется перед тем как запускается экшен контроллера, который указан в параметрах(по умолчанию это все/каждый экшен)
...
class QuestionsController < ApplicationController
  ...
  # (если мы используем несколько фильтров то их порядок важен в том случае если данные из методов отдих фильтров взаимодействуют с другими. ХЗ почему так сделано ??)
...
# concerns - отдельные фаилы с модулями(имеющими дополнительные способности) содержащими обработчики ошибок, они создаются в controllers/conxerns/
