...
# before_action  -  это действие(метод) который исполняет переданный в параметры метод контроллера, перед тем как запускается каждый экшен контроллера, так же указанный в параметрах (по умолчанию это все экшены)
...
class QuestionsController < ApplicationController
  ...
  # only: %i[destroy update]   -  экшены перед которыми будет запускаться метод set_question! Если не указывать данный параметр, то before_action будет выполнять метод перед всеми/каждым экшеном
  ...
  # Если мы используем несколько фильтров то их порядок важен в том случае если данные из методов отдих фильтров взаимодействуют с другими.
  ...
  def set_question! # метод который будет запускаться при помощи before_action, называем с восклиц знаком в конце, тк он может вернуть ошибку если не существует такой вопрос
    ...
    # Метод find для этой цели лучше find_by, тк если пользователь введет адрес несуществующего вопроса, например /questions/266666, то возникнет ошибка ActiveRecord::RecordNotFound в контроллере(в этом методе), а не ошибка в виде, соотв проще будет эту ошибку обработать
  end
end
...
class ApplicationController < ActionController::Base
  ...
  # rescue_from  -  метод спасает ошибку из параметров и далее исполняет метод ее обработки
  # ActiveRecord::RecordNotFound  -  параметр, имя ошибки
...
# concerns - отдельные фаилы с модулями(имеющими дополнительные способности) содержащими методы, например обработчики ошибок, они создаются в controllers/conxerns/ если содержат методы, применяемые в контроллерах (чтоб не захламлять контроллеры ??)

# Вариант 2: Создадим отдельный фаил controllers/concerns/error_handling.rb, который содержит модуль с необходимыми методами и обработаем ошибку в нем, а в материнский контроллер его просто подключим
class ApplicationController < ActionController::Base
