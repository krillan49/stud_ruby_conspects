...
# (?? как варик и для Синатры ??) так же можно создавать отдельные виды для каждого языка и возвращать их в соответсвии с тем какой язык выбрал пользователь(хранится например в таблице users)
'some.ru.html.erb' # в название фаила вида добавляем расширение с обозначением языка
...
locale                    # Получить или установить(чкрез =) текущую локаль (знвчения это :en, :ru итд)
...
# Пример локализации(форматирования) даты и времени l10n для разных локалей с использованием гема rails-i18n, в методе декоратора
# Изменим метод formatted_created_at в decorators/question_decorator.rb
...
class ApplicationController < ActionController::Base
  ...
  def switch_locale(&action) # &action - обязательный парамер, это действие(экшен) контроллера, которое необходимо выполнить
