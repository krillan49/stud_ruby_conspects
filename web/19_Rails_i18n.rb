...
# (?? как варик и для Синатры (без yml) ??) так же можно создавать отдельные виды для каждого языка и возвращать их в соответсвии с тем какой язык выбрал пользователь(хранится например в таблице users)
...
# Все наши строки переводов будут находиться в отдельных фаилах yml в директоии config/locales, а в представлениях будут заглушки обозначающие их; и при рэндере страницы, строка перевода будет подставлена на место заглушки в зависимости от выбранной пользователем локали

# Можно создавать перевод для сайта и вызывать константы с переводом во views.
...
# 2. Создадим в фаиле /config/locales/en.yml раздел, например contacts с подразделом и строковым значением перевода:
...
# В Gemfile можно(рекомедуется, далее используется) подключить гем для наиболее типичных переводов, чтобы не прописывать в ручную: название месяцев, дней недели, валют, типичные ошибки валидации итд. https://github.com/svenfuchs/rails-i18n (переводы можно посмотреть в директории гема rails/locale)
gem 'rails-i18n', '~> 7'
# > bundle install
...
# Можем создавать любые каталоги(это как пространства имен) с любой структурой и по любым обобщениям которые нам нужны (например по страницам, по темам, по фичам итд)
# Удобно формировать каталоги согласно файловой структуре приложения, чтобы использовать краткую форму записи в видах


# Атрибуты(каталоги) для имен столбцов таблиц из БД(по умолчанию используются в ошибках валидации, формах и других местах например в своем выводе этих названий), в yml будут иметь структуру:
'activerecord:
  attributes:
    model_name:
      col_name: Перевод имени столбца'
# 1е 2 каталога неизменяемы, 3е название модели(в единственном числе) и 4е название столбца для которого делаем перевод, названия будут такими же как в схема.рб


# Перевод названий моделей(в томже базовом каталоге)
'activerecord:
  models:
...
# Работа со множественным числом в разных языках на примере сообщения об ошибках (правила уже созданы в геме rails-i18n, нужно создать правильные ключи)
# en.yml
'global:
  forms:
    messages:
...
# Создание части путив переменной, чтобы не дублировать пример в admin/users/index.html.erb
scope = 'activerecord.attributes.user' # переменная для того чтобы не дублировать путь к переводу
t 'name', scope: scope  # scope: scope - берем переменную и добавляем к ней '.name'
...
locale                    # Получить или установить(через =) текущую локаль (знвчения это :en, :ru итд)
...
# В Рэилс при соответствии маршрута разделов в yml и последовательности каталогов в приложении(контроллер.представление.ключ для вида, либо контроллер.экшен.ключ для экшена в контроллере гапример перевод флэш сообщений) то вместо этой полной последовательности:
...
# У пайджи есть и своя реализация i18n, но в случае Рэилс и свой встроенный норм
...
# Задача в том чтобы адрес мог содержать как нащши локали(localhost:3000/en/posts или localhost:3000/ru/posts) так и был адрес без локали, в котором будет локаль по умолчанию(localhost:3000/posts)
Rails.application.routes.draw do
  # оборачиваем все маршруты в scope:
...
# 4. Метод и фильтр для считывания локали из URL каждого запроса для экшенов(экшены теперь должны обрабатывать новые маршруты c локалью языка), для дальнейшего подключения нужного языка в возвращаемые этим экшеном представления.
# Добавляются в ApplicationController, а остальные наследуют
class ApplicationController < ActionController::Base # в след разделе вынесем в консерн
  ...
  def switch_locale(&action) # &action - обязательный парамер, это действие(экшен) контроллера(блок со всем кодом из соотв экшена ??), которое необходимо выполнить
    ...
    # locale_from_url - подметод чтобы взять локаль из URL запроса, что мы настроили в маршрутах "(:locale)"
    ...
    # with_locale - метод который говорит что действие контроллера будет выполняться в контексте запрошенной локали
  end
  ...
  # Для того чтобы локаль, которая установлена на данный момент пристыковывалась к каждой ссылке по которой мы переходим, те сохранялась между запросами, тоесть при переходе на другую страницу сохранялся выбранный язык
...
# или (AskIt) в _menu.html.erb
# (!!! Почемуто не срабатывают стили с картинкой языка, фаил стилей видит, сами стили тоже, картинка почемуто не видна и находится за фоном шапки мб чето не то с юрл или самой картинкой ??)
...
# (?? хз откуда это. настройки языка для пользователя можно либо в куки положить, либо в базу(создать отдельный столбец в users))

# Козда бразер отправляет запрос на сервер, то в заголовках запроса есть данные о предпочитаемой пользователем локали
...
# но сам гем использовать не будем а просто позаимствуем часть кода

# Заодно сразу вынесем код и материнского контроллера в новый консерн internationalization.rb
