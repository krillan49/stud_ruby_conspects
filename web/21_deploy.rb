...
# https://github.com/krdprog/prog-conspects/blob/master/hartl-rails.md  -  инструкция развёртывания на Heroku(?? Больше нет бесплатного)
...
# Render - хостинг, который предоставляет услугу PaaS (platform as a service). С тех пор, как Heroku убрал бесплатные тарифные планы, Render может рассматриваться как его замена, так как он позволяет быстро и легко развернуть своё приложение, ничего при этом не платя. Мы посмотрим, как сделать деплой приложения Rails, настроить БД Postgres, добавить Redis, настроить обработку фоновых задач с Sidekiq и реализовать выполнение заданной на задаче на регулярной основе (по типу Cron).
...
# NEW+ => PostgreSQL => Из настроек обязательно выбираем имя, регион(лучше франкфурт тк ближе) и тарифный план.
...
# 2. Создание Redis БД (например для экшен кейбл турбостримов или сайдкик нужен)
# NEW+ => Redis => Тоже имя и регион, а так же настройку Maxmemory Policy: просто для кэша подойдет и allkeys-lru(recommended for caches), а если юзаем задачи в очередях, сайдкик, турбостримы, экшенкэйбл то лучше noeviction(recommended for queues/persistence), тк не будет удалать старые задачи.
# В бесплатном тарифном плане всего 50 подключений, что очень мало, тк например экшенкэйбл тратит 1 на 1 юзера
# Далее в панели Dashboard можно посмотреть информацию о созданной БД(создается какоето время)
...
# Gemfile - должен быть гем именно постгресс(как минимум в группе продакшен), а не скюлайт3
gem 'pg', '~> 1.4.3'
# (!! Для виндоус) Gemfile.lock - удостоверимся что прописана правильная платформа
"PLATFORMS
  x64-mingw-ucrt
  x86_64-linux" # дописываем эту строку если ее нет, без нее на сервере гемы не встанут, тк на сервере линукс
# config/database.yml - удостоверимся что в дефолтных настройка адаптер postgresql
'default: &default
  adapter: postgresql'
...
# config/puma.rb - удостовериться, что есть(раскомментирована) настройки:
workers ENV.fetch("WEB_CONCURRENCY") { 2 }
preload_app!
# config/envirnoments/production.rb - ищем строку:
config.public_file_server.enabled = ENV['RAILS_SERVE_STATIC_FILES'].present?
# и добавляем в нее  || ENV['RENDER'].present?:
...
# NEW+ => Web Servise => Build and deploy from a Git repository => Next => Connect Repository => (инсталируем гитхаб или гилаб если еще не было) конектимся к нужному репозиторию => В настройках выбираем так же название и регион, The runtime for your web service (среду выполения) Ruby, ветку в репозитарии, Build Command - можно оставить то что было по умолчанию, но лучше используем наш шеловский фаил bin/render-build.sh те пропишем в поле команду ./bin/render-build.sh; Start Command - можно по умолчанию. Ну и выюерем тариф
# Так же в самом низу нужно будет добавить переменные среды(для БД)
...
# В Меню дпнного веб сервиса есть:
...
# NEW+ => Background Worker => Аналогично веб сервису выбираем нужный репозитарий => все настройки теже, только для Build Command можно просто вписать bundle install тк для фоновых задач особо ничего не нужно типа эссетов итд; а в Start Command если мы например используем сайдкик то так и можно вписать bundle exec sidekiq
