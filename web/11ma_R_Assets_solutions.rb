...
# Тоесть webpacker это довольно сложное решение, одновременно делает и бандл и транспайл и прекомпиляцию css и еще и отдавать все эти ассеты клиентам. Сложные настройки, сложные миграции на другие версии, тоесть у новичка чтото постоянно будет ломаться
...
# Открыть config/application.rbи удалить config.assets.paths << Rails.root.join('app','assets');
