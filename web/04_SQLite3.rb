...
db = SQLite3::Database.new('test.sqlite') # подключаем/создаем базу данных с именем test.sqlite как объект класса Database, прописывая к ней путь и присваиваем ее в переменную. Если этот фаил уже создан то откроет его, а если не создан то создаст с этим именем в текущем каталоге приложения(можно поменять путь на любой).
# Расширения для фаила: sqlite, sqlite3, db ...

# execute - метод, который принимает аргументом строку запроса и исполняет ее.
# точка с запятой в конце команды необязательны тк execute подразумевает выполнение только одной команды.
...
# вывод из БД:
db.execute "SELECT * FROM Cars" do |car| # execute принимает блок и присваивает результаты запроса(таблицу построчно в виде массива по колонкам) в переменную как итератор.
	p car    #=> [1, 'Kroker', 9000]
end
...
# Если строка окажется содержащей ', то это завершит строку в операторе SQL, что вызовет ошибку; или что еще хуже, если злоумышленник сделает эксплойт SQL-инъекции. Cледует избегать построения SQL-запроса путем интерполяции или добавления строк; исправлять это способом отфильтровывания или цитирования неправильных символов, будет довольно сложно.
