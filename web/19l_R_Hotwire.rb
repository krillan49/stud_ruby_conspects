...
# Turbo - фрэймворк, который позволяет разбивать веб-страницы на отдельные независимые контексты и обновлять/перерисовывать только необходимые части веб страниц, не перерисовывая всю страницу. Тоесть добавляет ассинхронную функциональность страницам, что ускорять работу приложения.
...
def create
  ...
  if @question.save
    respond_to do |format|
      ...
      # Добавим формат turbo_stream, это тот формат в котором тут принимаем форму, что была обернута в турбострим-тег
...
# ?? Почему для формы вопроса используем turbo-frame тег, а в лэйауте обычный div - тк тут хотим чтобы тег появлялся только при турбо стриме, а там в любом случае ??
...
# 5. Динамическое удаление вопросов, при нажатии кнопки "Delete" вопрос на странице динамически удалялся без перезагрузки страницы. Кнопку удаления, ктоме добавления turbo_ преффиксов, изменять не нужно.
...
# b. questions_controller.rb - добавим опцию для создания значения data-turbo-frame на кнопках пагинатора, соотв айдишнику фрэйма
def index
...
# 8. Исправим ссылку-заголовок на вопрос. На странице вопроса, добавим тот же рендер вопроса, что и на странице всех вопросов, вместо отдельного кода, тк так заодно будет работать турбофрэйм вызывающий форму редактирования.
...
# c. answers(turbo)/_answer.html.erb - обернем паршал ответа в турбофрэйм-тег с айди от конкретного овета, заодно удалим лишние опции с dom_id
...
# 10. Сделаем динамическими создание и удаление комментариев к вопросам и ответам на странице questions(turbo)/show.html.erb
# a. answers(turbo)/_answer.html.erb  - уберем передачу html_id, тк теперь будем его генерить в паршале _commentable.html.erb
# b. comments(turbo)/_commentable.html.erb  - добавим турбофрэймы и оптимизируем использование переменной html_id, которую теперь будем создавать тут в паршале а не передавать из _answer.html.erb или show.html.erb
# с. comments(turbo)/_comment.html.erb  - обернем коммент в турбофрэйм тег
# d. questions(turbo)/show.html.erb  - уберем передачу html_id, тк теперь будем его генерить в паршале _commentable.html.erb
