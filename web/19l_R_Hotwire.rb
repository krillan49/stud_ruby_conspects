...
# 3. При создании вопроса при помощи турбо стрима не отображается флэш-сообщение, добавим возмоность отображения флэша тоже динамически
...
# 8. Исправим ссылку на вопрос на странице всех вопросов. И на странице вопроса, добавим тот же рендер вопроса, что и на странице всех вопросов, вместо отдельного кода, тк так заодно будет работать турбофрэйм вызывающий форму редактирования.
...
# b. comments(turbo)/_commentable.html.erb  - добавим турбофрэймы и оптимизируем использование переменной html_id, которую теперь будем создавать тут в паршале а не передавать из
...
class CommentsController < ApplicationController
  ...
  def destroy
    # Тут везде слелаем @comment те переменную класса(иначе будут проблемы с отображение флэша), тк она будет использоваться в паршале destroy.turbo_stream.erb
