...
# Turbo - фрэймворк, являеющийся частью фрэймворка Hotwire. Появился как продолжение идеи turbolinks, но намного более мощный. Он позволяет ускорять работу приложения, тк позволяет разбивать веб-страницы на отдельные независимые контексты и обновлять/перерисовывать необходимые части веб страниц, не перерисовывая всю страницу. Тоесть добавляет ассинхронную функциональность страницам
...
# 1. Динамическое отображение формы:
...
# Тоесть теперь без перезагрузки страницы index.html.erb мы подгружаем в нее форму из _form.html.erb


# 2. Сделаем так чтоб при создании нового вопроса вместо редиректа на index.html.erb, происходила подгрузка разметки этого вопроса к списку вопросов без перезагрузки всей страницы
# а. questions_controller.rb - изменим экшен create разбив его на форматы
def create
  @question = current_user.questions.build question_params
  if @question.save
    respond_to do |format|
      # Оставим обычный html формат, на случай если турбо-стрим не работает, например когда скрипты отключены у пользователя
      format.html do
        flash[:success] = t('.success')
        redirect_to questions_path
      end
      # Добавим формат turbo_stream, это тот формат в котором принимаем форму
...
# b. questions(turbo)/create.turbo_stream.erb  - создадим этот специальный фаил, который будет рендерится в ответ на формат turbo_stream
