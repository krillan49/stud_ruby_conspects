...
# > git add .                      - добавить все фаилы и папки проекта(кроме .git) в стадию ожидания комита. Добавляется только если есть чтото отличное от последнего коммита
...
puts '                            GitHub(загрузка данных из удаленного репозитория)'
...
# 1. Fork: Заходим на необходимый чейто репозиторий, нажимаем Fork справа вверху, создаем себе копию репозитория, клонируем, вносим изменения, пушим. Далее автору ставятся от этом пометки в поле fork.
# 2. Pull Request: Нажимаем на Contribute справа вверху(под зеленой кнопкой Code) -> Open Pull Request откроются настройки(из какой ветки какого репозитория в какие, все изменения)
# 3. Pull Request: Нажимаем Create Pull Request и указываем комментарии к изменениям которые внесли, нажимаем Create Pull Request и теперь на вкладке Pull Request у нас появилась отметка
# 4. Автор: приходит уведомление(на вкладке с колокольчиком), открываем смотрим(кнопка Review changes - написать ответ) Merge Pull Request - слияние репозиториев(принять пул реквест)
