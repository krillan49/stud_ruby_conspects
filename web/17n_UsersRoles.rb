...
# pundit оперирует таким понятием как политика, которая является классом в отдельном фаиле, который описывает что пользователь может делать с определенным ресурсом(вопросами, ответами, юзерами итд)
...
class QuestionsController < ApplicationController
  ...
  def authorize_question!
    ...
    # authorize - метод pundit, он проверяет имеет ли пользователь право на действие с соответсвующими контроллерами относящимися к данной модели. Для этого данный метод создает объект класса QuestionPolicy(тут)
...
class GuestUser
  ...
  # Гостевой юзер должен уметь отвечать на все методы юзера(методы ролей, метод author? итд) тоесть эти методы тут нужно завести:
...
# В comments_controller.rb сделаем авторизацию прямо в экшенах, без фильта и метода, тк всего 2 экшена
