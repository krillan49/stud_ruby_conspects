...
# 6. Но когда мы обновляем пользователя, в модели user.rb производятся валидации, в том числе correct_old_password, которая проверяет старый пароль, но тк админ не знает старый пароль, изменим эту валидацию чтобы для админа она была не обязательна
class User < ApplicationRecord
  enum role: { basic: 0, moderator: 1, admin: 2 }, _suffix: :role
...
puts
puts '                                    Pundit. Политика для ресурса вопросов ч1'
...
class QuestionsController < ApplicationController
  ...
  def authorize_question!
    ...
    # authorize - метод pundit, он проверяет имеет ли пользователь право на действие с соответсвующими контроллерами относящимися к данной модели. Для этого данный метод создает объект класса QuestionPolicy(тут) и по умолчанию для того чтобы передать параметр юзера используется метод с названием current_user, который мы ранее создали, можно конечно прописать, чтоб вызывался другой, но проще называть хэлпер текущего юзера именно current_user
...
# Разрешим некоторые экшены некоторым польхователям в question_policy.rb
class QuestionPolicy < ApplicationPolicy
  # ...
...
class User < ApplicationRecord
  ...
  # Метод проверяет является ли автор владельцем объекта(вопроса, ответа или коммента)
  def author?(obj) # сделаем метод универсальным для вопросов, ответов и комментов
    obj.user == self # тоесть юзер которому принадлежит сущьность это тот юзер что вызывает метод(тоесть текущий)
  end
end
# Так же сделаем метод обратный методу author?, который будет проверять, написан ли вопрос ответ или коммент конкретным юзером. Для этого создадим отдельный консерн в моделях models/concerns/authorship.rb (код там), и подключим этот консерн во все соответсвующие модели
include Authorship # подключим в модели Question, Answer, Comment


puts
puts '                          Объект гость/Гостевой юзер. Политика для ресурса вопросов ч2'

# Если мы будем заходить на метод update с незарегистритованного пользователя, то возникнет ошибка undefined method `admin_role?' for nil:NilClass, тк метод admin_role? вызывается от user, но у незарегистрированного гостя нет юзера и current_user вернет nil от которого и вызывается метод и происходит ошибка
...
# Создадим новй сервисный объект services/guest_user.rb (полный код есть и в нем)
# Так же добавим экземпляр гостевого юзера в application_policy.rb, чтобы собственно был объект в переменной user от которой вызываем методы
class GuestUser
  def guest? # создадим в нем метод определяющий гостся как true
    true
  end

  # Так же гостевой юзер должен уметь отвечать на все методы юзера(методы ролей, метод author? итд) тоесть эти методы тут нужно завести:
...
# При этом в модели user.rb создадим точно такой же метод только со значением false
class User < ApplicationRecord
  def guest?
    false
  end
