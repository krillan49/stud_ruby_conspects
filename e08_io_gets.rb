...
gets # Можно использовать гетс и без предачи ее в переменную, например для подтверждения исполнения дальнейшего кода
...
# При помощи .to_i 27.5+22=29 поэтому для числе с плав точкой переводим в тд .to_f
puts ("Введите 1е число: ")
m = gets().chomp()
puts ("Введите 2е число: ")
n = gets().chomp()
...
# причем ошибка будет и без puts  тк руби сперва считает выражение а потом передает его оператору вывода
