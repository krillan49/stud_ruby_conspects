...
# PUT, DELETE отправляются через POST, но с добавлением скрытой переменной.
...
# Можно передать данные(оч небольшие), но эти данные будут в строке запрса(после знака ?), например:
# /cart?product_1=4,product_2=7,product_3=4,
# Пример из поисковика(после ? мдут параметры, которые разделяются знаком &)
# www.yandex.ru/search/?text=запорожская+аэс&lr=37129&search_source=chromentp_desktop&src=suggest_Pers
# Это результат того что форма указывает на гет-запрс <form aсtion="GET"...
