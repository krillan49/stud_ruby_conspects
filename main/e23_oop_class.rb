...
class MyAnimal # Классы называются именами существительными в единственном числе, каждое слово с большой буквы без разделения пробелами.
...
class Song
  ...
  def duration=(new_duration) # 2й споcоб, чкркз метод имя которого оканчивается на =
    @duration = new_duration
  end
