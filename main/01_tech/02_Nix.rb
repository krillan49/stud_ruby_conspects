...
# «Системный» руби  - это тот Руби, который находится в директории /usr/bin

# Проверить где находится исполняемый файл ruby(при помощи которого запускаются программы в терминале, например "ruby app.rb") в системе без менеджеров версий (например rvm):
# $ which ruby            => /usr/bin/ruby
# Другими словами, интерпретатор руби находится в /usr/bin/ruby. Это бинарный файл, его можно просмотреть с помощью, например, Far Manager. Если удалить этот файл, то руби-программы запускаться не будут
