...
local_var = 'some'
# переменная доступная только внутри метода(или внутри той области кода, в которой она объявлена), можно определить(объявить) только в том же методе.
...
# 2. instance variable/переменная экземпляра класса
@instance_var = 'some'
# доступня во всех методах класса(или методах той области в которой объявлена вне класса), можно определить в любом методе класса. Эти переменные определяют состояние объекта. Желательно объявлять instance variables в конструкторе.
