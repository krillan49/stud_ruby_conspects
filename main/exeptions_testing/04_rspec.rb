...
# При помощи expect и матчеров можно задать конкретную инструкцию проверки для RSpec теста и в выводе будет более подробный ответ что конкретно пошло не так, без необходимости писать дополнительный код с исключениями в тесте
RSpec.describe 'this is a testing suite' do
  it 'self.run' do
    result = Demo.run
