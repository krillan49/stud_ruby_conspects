...
# При помощи expect и матчеров можно задать конкретную инструкцию проверки для RSpec теста и выдаст более подробный ответ что конкретно пошло не так без необходимости писать дополнительный код с исключениями в тесте
RSpec.describe 'this is a testing suite' do
  it 'self.run' do
    result = Demo.run
...
# Передача блоком, а не аргументом в expect при необходимости
expect do
  post('/translate/yoda.json', test_client, text: nil) # код который породит необходимое исключение
end.to raise_error(
  described_class::BadRequest, 'Bad Request: text is missing.'
)
# raise_error - матчер проверяющий что в блоке expect было порождено исключение, класс которого соответсвует переданному в параметр, а сообщение переданной строке или регулярке во 2й параметр, если не передать параметрры, то проверит просто на то что какое-то исключение порождено, но так делать не рекомендуется


# expect .to output - проверяет что метод выводит чтото в термилнал, обычно это stdout
...
# described_class - метод RSpec возвращает константу тестируемого класса или модуля, если она была передана аргументом в метод describe из тела которого вызывается метод
