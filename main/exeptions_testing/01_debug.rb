...
# Для Ruby on Rails, полезна будет следующая конструкция:
something = '123'
...
puts random_pow
# раньше мы запускали pry непосредственно из командной строки. А в этот раз мы запустили программу обычным способом:
#...
# $ ruby app.rb
...
# 0
#...
# Стрелка(=>) слева показывает где мы находимся. Если ввести команду whereami — то покажет место текущего брейкпоинта.
...
# Другие возможности(команды) pry, когда мы находимся в точке остановки:
next # выполнит следующую строку. После этого снова можно посмотреть значение переменных, чтобы понять что не так с программой.
exit # вернет выполнение в программу. Т.е. это выход из Pry, но не из программы. Можно было дать более правильное имя этой команде continue (продолжить).
exit! # прервет выполнение программы с выходом в терминал.
whereami # (от англ. Where Am I — где я?) может быть полезна когда, например, после вывода большого текста на экран (или очистки экрана кодом из нашей программы) мы все еще хотим знать в каком месте программы мы в данный момент находимся.
