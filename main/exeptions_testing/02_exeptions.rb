...
def some(par)
  begin
    10 / par
  rescue ZeroDivisionError => var # Присваивам экземпляр класса ZeroDivisionError в переменную, имя переменной можно задать любое
    p var
    p var.class
  rescue => other
    p other
...
# (??  почемуто так не обрабатывает все ошибки и получается бесконечный цикл. Не запоминает код из блоков обработку предыдущих ошибок ??)
n, m = 0, 'A'
begin
  a = 10 / n
  b = 2 + m
...
# raise или fail - оператор вызова(создания/формирования) собственного исключения, опционально могут быть добавлены текст с сообщением, тип исключения и информация о стеке вызовов

# без аргументов повторно вызывает текущее исключение(или RuntimeError, если текущего исключения нет)
raise() #=> unhandled exception

# С аргументом строкоцй - создает новое исключение RuntimeError, устанавливая аргумент в свойство сообщения. Затем это исключение поднимается вверх по стеку вызовов.
...
# Можно спасать несколько исключений в 1й строке
...
# Помимо встроенных классов исключений можно создавать и использовать свои собственные классы исключений, для обработки случаев, которые мы сочтем ошибками
# Если мы создадим свои классы исключений, они должны быть подклассами либо класса Exception, либо одного из его потомков(Interrupt,  NoMemoryError,  SignalException,  ScriptError,  StandardError,  SystemExit.)

class Error < StandardError # Создаем новый класс исключений, который наследует у StandardError
  ...
  # Константа с хэшем констант для удобного вызова ошибок по номеру
  ERRORS = {
  ...
  # Соотв этот метод будет унаследован всеми выше созданными классами
...
# Например сервер возвращает ответ из которого мы достаем код ошибки и тело с сообщением про ошибку и передаем в данный метод
def error_from_server(code, body)
  # Создаем исключение в ручную от нашего базового класса Error, если сервер передал код, которого нет в хэше ERRORS
  raise(Error, body) unless FunTranslations::Error::ERRORS.key?(code)
