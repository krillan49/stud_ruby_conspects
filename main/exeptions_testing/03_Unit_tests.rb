...
# Существует большое количество тестов для программ: ручные тесты, автоматизированные, юнит-тесты (модульные тесты), интеграционные тесты, нагрузочные

# тесты, с которыми чаще всего встречается программист: это юнит-тесты / модульные тесты (unit tests, от англ. «unit» — модуль или часть)

# это удобно: разработчик изменил программу, запустил тесты и проверил, что ничего фундаментального не сломалось. Если сломалось, то тут же исправил. В итоге, от появления ошибки до момента ее выявления прошли минуты. на относительно небольшом проекте с 500 тестов на каждые 100 изменений будет запущено по 500 тестов на каждое изменение, что в общей сложности дает 50 тысяч запусков разных тестов. Этот подход позволил значительно улучшить качество написанных программ. Однако, у юниттестирования есть и недостатки. Во-первых, вместе с написанием кода программы программисты также должны писать тесты. Во-вторых, юнит-тестирование никогда не покрывает абсолютно все участки кода. Десять конструкций if...else уже дают 1024 (двойка в десятой степени) возможных вариантов выполнения вашей программы. В некоторых проектах используют такое выражение как «покрытие кода» (code coverage), которое выражается в процентах. Например, говорят: «code coverage для нашего проекта составляет 80%» (при этом это является предметом гордости). На самом деле вопрос в том, как считаются эти проценты. Да, отдельные модули могут быть покрыты тестами. Но даже 100% покрытие не является панацеей от абсолютно всех ошибок: возможных вариантов выполнения программы всегда во много раз больше, чем тестов, которые может написать человек. В-третьих, существует особенность о которой редко говорят. Юнит-тесты обычно пишут сразу после написания какого-либо кода. Но на начальном этапе программный дизайн функциональности обычно еще не зафиксирован. Юнит-тесты фиксируют дизайн программы на этапе, когда дизайн еще достаточно свеж и может поменяться. Если поспешить с написанием юнит-тестов, то есть вероятность того, что тесты нужно будет переписывать.

# Тесты в идеале не должны знать ничего о программе и никак от нее не зависеть, они должны получать только какие-то данные на вход и проверять их с чем-то. Так при рефакторинге не придется их переписывать

# Тесты помогают при рефакторинге, так мы меняем код, но не меняем его функциональность


# Тк тесты это дополнительный код и соответсвенно увеличение затрат и замедление разработки, то нужно понимать когда и где их нужно писать:
# 1. Методы классов удобно тестировать, тк они как правило они делают что-то конкретное. Эти тесты пишутся довольно быстро
# 2. Если найден и исправлен какой-то баг, то стоит написать на этот код тест, чтобы этот баг снова обнаружить если он появится снова при доработке программы
# 3. Проверка граничных/крайних условий
# 4. Проверка мутных и сложных мест в коде


# абсолютно для каждого языка программирования существует фреймворк для создания тестов (очень часто таких фреймворков существует сразу несколько)

# вопрос читаемых тестов это не вопрос инструмента, а вопрос баланса smart vs simple

# Есть разные подходы к тестированию, например:
# Писать сначала программу, а потом тесты (TDD)
# Сначала создавать тесты, а потом программу (т.н. Test Driven Development, TDD)
