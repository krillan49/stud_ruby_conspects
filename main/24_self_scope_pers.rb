...
# Каждое определение метода/модуля/класса называется воротами области, поскольку создается новая область. Старая область видимости вам больше не доступна, а все доступные в ней переменные заменяются новыми.

v0 = 0
class SomeClass # Scope gate
  v1 = 1
  p local_variables #=> [:v1]
  # метод local_variables влзвращает все локальные переменные в области
...
# Область действия класса была заменена областью действия метода экземпляра, что привело к появлению нового набора переменных

# v0 нигде не появился - в тот момент, когда мы вошли в class SomeClass, эта область верхнего уровня также была заменена(временно, а не навсегда).
p local_variables #=> [:v0]


puts
puts '                                        "Слом" границ области'

# Если мы хотим иметь доступ к переменным или методам из другой области, то замените элементы области вызовами методов. Точнее, заменить:
# определения классов с Class.new
# определения модулей Module.new
# определения методов define_method
