...
# local_variables - метод, который возвращает все локальные переменные в области
p local_variables #=> [:a]
...
class SomeClass # Scope gate - когда мы вошли в класс, область верхнего уровня была заменена(временно).
  ...
  def some_method # Scope gate - Область действия класса была заменена областью действия метода экземпляра, что привело к появлению нового набора переменных
    v2 = 2
...
# Чтобы блоки не изменяли внешние переменные, нужно их продублировать блочными локальными переменными.
...
res = []
2.times do
  # Тут при каждой новой итереции переменной i снова не существует (те выдаст ошибку при вызове)
...
puts
puts '                                      Замыкание и блочные объекты'

# Блоки и блочные объекты (процессы, лямбды) видят область действия в своем определении, а не при вызове. Это связано с тем, что в Ruby они рассматриваются как замыкания.
