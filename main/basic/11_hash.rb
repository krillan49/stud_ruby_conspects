...
# Очень просто. Запомни, что хеш это массив. Для того чтобы попасть в массив, нужен индекс элемента. Нужен способ превратить любое значение(строка например) в число(которое будет использовано как индекс). Этот способ - это хеш функция. Хеш функция примает что угодно(например строку) и выдает число. Одна и та же строка всегда будет превращена в одно и то же число. Это число будет использовано как индекс в массиве и по этому индексу будет записана эта строка.
...
# Итого - берем что угодно, пропускаем через хеш функцию, получаем индекс в массиве, пишем туда элемент(или проверяем есть ли он там)
