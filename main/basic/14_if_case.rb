...
# 2. case in - проверяет принадлежность нескольких значений. Нужно соблюдать порядок расположения
def quadrant(x, y)
  case [x, y]
    in [0.., 0..] then 1
    in [..0, 0..] then 2
...
# 3. case-when можно запустить без передачи в case аргумента, но с дополнением условия в поле when, тогда выполнится первое when которое вернет true. Так будет возможно выполнение логических операций и соответсвенно операций сравнения(напр <, >=).
num = 11
case
when num.odd?
  puts 'odd'
