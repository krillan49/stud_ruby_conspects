...
# Условный оператор тоже возвращает, так что можно применять к нему сщщтв методы
if stack.size == 0 || %w[N NE E].include?(stack[-1])
  [
    %w[+ |].include?(hh[[y+1, x]]) ? hh[[y+1, x]] : nil,
    %w[X /].include?(hh[[y+1, x-1]]) ? hh[[y+1, x-1]] : nil,
    %w[+ -].include?(hh[[y, x-1]]) ? hh[[y, x-1]] : nil
  ]
end.compact


# Логические операторы  > < со строками проверить ?


puts '                                          Операторы if'

# Операторы if должны всегда использоваться для определения верно ли условие - true или false
...
# &&(and) - логическое и
# ||(or) - логическое или
# !(not) - конвертирует истинное в ложное и наоборот
...
x>0 ? (y>0 ? 1 : 4) : (y>0 ? 2 : 3) # условие в условии(скобки не обязательны)
...
when 2
  nameOfDay = "Вторник"
when 3 # Для перехода от условия у исполняемому коду можно использовать 1.перевод строки ...
  nameOfDay = "Среда"
when 4 then nameOfDay = "Четверг" # ... 2.Оператор then ...
when 5; nameOfDay = "Пятница" # ... либо 3.Точку с запятой
else # Оператор else для оператора case when работает так же как и для оператора if
  nameOfDay = "Выходной"
end
...
# Альтернативный вариант записи без аргумента. Тогда выполнится первое when которое вернет true.
x = 1
case
