...
# Вызов через []
def test(param = nil)
  result = param || -> { 'param is required' }
end
a = test()
p a[]


# Главное помнить, что это все одно и то же с минорными различиями. Это просто функции. Блок это просто функция(с особым синтаксом и минорными отличиями).


# Название лямбд от символов, например чтобы добавить в хэш (??или вызфвать в методах так map(:method) потом проверить)
def initialize(a, b)
  {
    add: -> { a + b },
    multiply: -> { a * b },
