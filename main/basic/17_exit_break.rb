...
# Перезапускает(повторяет) эту итерацию самого внутреннего цикла без проверки состояния цикла. Перезапускает yield или call , если вызывается внутри блока
for i in 0..5
	if i < 2 then
...
# Пример(хз почему не идет дальше 1 ??):
for i in 0..5
	if i < 2
		print "is #{i}, "
    n = i if !n
