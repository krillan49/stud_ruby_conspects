...
puts '                                     File.new (Создание/чтение фаила)'

# File.new - метод для открытия/создания фаила (объект File), не может принимать блок. Подходит для чтения, записи или того и другого, в зависимости от строки режима.
file = File.new("./text/simple.txt", "r") # присваиваем объект фаила в переменную

p file      #=> #<File:./text/simple.txt>    # метаданные объекта

# read - метод выдаст содержание фаила в формате строки, с которой можем проводить любые строковые операции
p str = file.read #=> "вася вася возвращаемся\nпетя пертя помещается\nколя коля коля\n"
...
p file.readline #=> "вася вася возвращаемся\n"
p file.readline #=> "петя пертя помещается\n"
p file.readline #=> "коля коля коля\n"
p file.readline #=> end of file reached (EOFError)
...
# readlines - метод, возвращает массив элементы которого строки фаила. Так же как и предыдущие читает только недочитанную часть фаила, те с той строки или символа на котором отсановилось чтение. Не вызывает ошибку для уже прочитанного фаила
p arr = file.readlines #=> ["вася вася возвращаемся\n", "петя пертя помещается\n", "коля коля коля\n"]
# Соответсвенно можем применять к результату методы массивов или циклы:
for line in arr
...
File.open("./simple.txt", "r") do |file| # в переменную присвоится указанный фаил.
  # можно аналогично использовать все методы что и выше в File.new
  p file           #=> #<File:./text/simple.txt>
  p file.readchar  #=> "в"
  p file.readline  #=> "ася вася возвращаемся\n"
  p file.readlines #=> ["петя пертя помещается\n", "коля коля коля\n"]
  p file.read      #=> ""
end



puts '                                        write (Запись в фаил)'
...
# 3. Запись в фаил помощи одного метода write(любые методы записи). Создаст новый фаил если такого нет
