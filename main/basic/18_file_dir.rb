...
# Если есть некий вывод в консоль и нужно сохранить его в фаилЮ например:
a = gets.strip
puts "Your word is #{a}"
...
# read - метод выдаст содержание фаила в формате строки, с которой можем проводить любые строковые операции
...
# readline - метод, читает фаил построчно в формате строки, если 1я строка уже прочитана, то прочитает следующую. Вернет ошибку если фаил уже до того полностью прочитан(построчно или при помощи read)
...
# sysread (метод системного чтения) - для чтения файла с возможностью указать колличество читаемых символов, указатель фаила останется на следующем символе. Можно открыть файл в любом из режимов. В случае 2хбайтовых символов чисто символов будет меньше, тк параметр это байты а не символы ??
file.sysread(7) #=> "Somethi"         # выведет первые 20 символов файла. Указатель файла теперь будет размещен на 21-м
file.read       #=> "ng\nSomethingSomething\nSomething"
...
# в режиме a+
output = File.open("./text/simple.txt", "a+")
p output.read #=> "Something\nSomething"
output.write "Something"
output.write "\nSomething"
p output.read #=> ""       # тк фаил уже перемотан в конец после записи, так былоб и без предыдущего чтения
output.close

output = File.open("./text/simple.txt", "r")
p output.read #=> "Something\nSomethingSomething\nSomething"
output.close
...
# 3. Запись в фаил помощи одного метода write(любые режимы записи). Создаст новый фаил, если такого нет
...
# chdir - метод изменения текущей директории директорию(можно указать и относительный и полный путь). Процесс, все его действия в файловой системе происходят относительно какого-то места, оно и называется текущим каталогом после запуска обычно оно равно местоположению программы
Dir.chdir "C:/Projects"
...
Dir.entries "E:/doc" #=> [".", "..", "pic", "ruby_exemples", "Гайд шатдаун DOXAхРоскомсвобода 27.02.2022.pdf", "ГВД", "з.txt", "мма", "паспорт", "покер", "текст", "учеба"]
# Особенности: "E:"(без слэша) == "E:/doc"
Dir.entries "E:/" #=> ["$RECYCLE.BIN", "doc", "games", "msdownld.tmp", "programs", "System Volume Information", "Безымянный.jpg", "видео", "загрузки", "музыка"]
Dir.entries "E:" #=> [".", "..", "pic", "ruby_exemples", "Гайд шатдаун DOXAхРоскомсвобода 27.02.2022.pdf", "ГВД", "з.txt", "мма", "паспорт", "покер", "текст", "учеба"]
