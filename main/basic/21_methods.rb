...
# &:  указатель на функцию, значит что переданная дплее функция применится к каждому элементую Работает только для 1го метода
...
# Три способа вызвать метод в Руби На примере метода object_id объекта класса Object
object = Object.new
...
def kroker_say2(word = "Я огромен!", num = 9000) # word и num - параметры, после = значения параметров по умолчанию. Все параметры/аргументы со значениями по умолчанию принято писать в конце
  puts "Я Крокер!"
...
kroker_say2(nil) #=> "Hello, "   # тоесть при передаче nil считается переданным аргумент и присваивает его а не аргумент по умолчанию
...
# Неопределенное колличество параметров прописываются после обязательных параметров
def sum(n, *args)
  args
end
p sum(1, '2', 3, 'gbcz') #=> ['2', 3, 'gbcz']

# Неопределенное колличество параметров, включая элементы хэша(прописываются последними)
...
def some
  ...
  @a = "B" # тут доп действие, после возврата
end
p @a #=> nil
p some #=> 6
p @a #=> "B"
...
def some(arg)
  ...
  'arg is not string' # возвращаем если в основном разделе(выше) ошибка
else
  res # возвращаем если нет ошибки
ensure
  @a = 5 # тут доп действие, активируется в любом случае (?? после возврата)
end
p @a #=> nil
...
puts
puts '                                     Хэш-подобная форма записи аргументов'

# Можно так же записать параметры как шаблон для хэша аргументов
def new_way(arg1:, arg2:, arg3: :test, arg4: 100) # обязательные ключи-аргументы оставлям без значений, в необязательные добавляем значения по умолчанию
  [arg1, arg2, arg3, arg4] # вызываем как переменные
end
# так при вызове порядок следования не важен
p new_way(arg2: "hello!", arg1: "hi") #=> ["hi", "hello!", :test, 100]
...
# Обычно принято называть options или params
...
# Если же мы принимаем 2 хэша то у 2го в операторе скобки писать обязательно, иначе будет непонятно где начинается 2й хэш
def met(a, hh, hh2)
  # Какой-то код
end
