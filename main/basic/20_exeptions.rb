...
rescue # Отслеживание на ошибку (Запускается если после begin выявлена какая либо ошибка)
...
puts
# (retry)Можно зафиксировать исключение, используя блок восстановления, а затем использовать оператор повторной попытки, чтобы выполнить блок begin с самого начала.
...
raise #=> unhandled exception  # просто повторно вызывает текущее исключение(или RuntimeError, если текущего исключения нет). Используется в обработчиках исключений, которым необходимо перехватить исключение перед его передачей.
raise "Error Message" #=> Error Message (RuntimeError)  # создает новое исключение RuntimeError , устанавливая его сообщение в заданную строку. Затем это исключение поднимается вверх по стеку вызовов.
raise ExceptionType # использует первый аргумент для создания исключения, этот тип исключения должен уже быть определен например RuntimeError иначе будет вызвано: uninitialized constant ExceptionType (NameError)
raise RuntimeError #=> RuntimeError (RuntimeError)
raise RuntimeError, "My Error Message" #=> My Error Message (RuntimeError)  # использует первый аргумент для создания исключения(этот тип исключения должен уже быть определен), а затем устанавливает связанное сообщение со вторым аргументом.
...
# ensure (гарантия) идет после последнего предложения rescue и содержит кусок кода, который всегда будет выполняться, когда блок завершается. Неважно, завершается ли блок нормально, если он вызывает и спасает исключение или завершается из-за неперехваченного исключения, блок ensure будет запущен.
begin
  raise 'A test exception.'
rescue Exception => e
  #.. обработайте ошибку
...
# Если присутствует оператор else, он идет после предложений rescue и перед любыми ensure. Тело предложения else выполняется только в том случае, если основная часть кода не вызывает никаких исключений.
begin
  #.. процесс
