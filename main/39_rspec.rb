...
describe Hero do # метод принимает тестируемый класс как название(название можно задать и просто строкой) и лямбду с тестами
  # Тест 1
...
# У нас часто повторяется код hero = Hero.new 'foo', и это не совпадает с DRY, потому оптимизируем, добавив before.
...
describe Hero do
  ...
  it("displays full hero info") { expect(@hero.hero_info).to eq "Foo has 100 health" }
end
...
# Весь доп код (мб кроме методов) стоит писать в хуках, соответсвующего дискрайба, а не просто в теле дискрайба
