...
# Блоки могут быть приняты методами, например чтобы добавить какую-то логику в определенный метод. Таким способом передать методу можно только 1 блок
function_name do # имя оператора метода/функции в который будет присвоен этот блок
  # ...            код блока
end


# yield - ключевое слово(на самом деле метод), он вызывает блок, который был передан в метод через пристыковывание к оператору этого метода, код из блока помещается/выполняется в месте где стоит yield. (это чтото вроде способа наполнения метода дополнительным кодом не в самом теле метода а вызванное при запуске метода)
def demo0(n1, n2)
...
def demo(*args)
  args.map do |n|
    yield(n, 5) # тут подставляется блок, например { |el, num| ... }, в переменную 'el' которого передается значение 'n', а в переменную 'num' число 5, в итоге блок исполнает свой код с этими параметрами тут
  end.sum
end
...
# Если перед последним аргументом метода стоит &, то в этот аргумент может и должен принять блок.
...
# lambda-функции(так же их называют lambda-выражения) - это, почти тоже, что и процедура, тоже отложенный блок кода, который можно передавать в качестве аргумента или по другому - (так же как и простые проки ?)это указатель на функцию у которой нет названия/имени
...
# Пример использования 4 (С мутными цепными вызовами):
def compose(add, id)
