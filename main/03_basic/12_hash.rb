puts '                                 Ассоциативные массивы / Hash'
...
# Использовать переменную с названием “hash” не рекомендуется, тк это зарезервированное ключевое слово языка
...
some = {
  ...
  # => - Hash rocket символ для обозначения принадлежности значения ключу, универсален, при помощи него ключами можно задавать любые объекты
	...
	# Двоеточие и рокет могут использоваться в одном хэше
...
# Стоит отдавать предпочтение ключам-символам, а не ключам-строкам
...
# 6. Это синтаксический сахар, добавленный в Ruby 2.7+ и поддерживаемый в Rails. Можно использовать такой сокращённый синтаксис только если переменные или методы(без аргументов) с такими именами уже существуют в области видимости. Ruby берет значения именно из локальных переменных (или методов без аргументов), доступных в текущем контексте
find_by(barcode:, provider:, cabinet:)
# означает то же самое, что и:
find_by(barcode: barcode, provider: provider, cabinet: cabinet)
# То есть Ruby автоматически подставляет значения переменных с такими же именами, как ключи хэша
