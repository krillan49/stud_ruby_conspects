puts '                         Функциональное программирование. Block, Proc и lambda'

# Block, Proc и lambda - это все примерно одно и то же, это просто функции с особым синтаксом и минорными отличиями друг от друга
...
# Блок/block - это кусок кода, что содержится между {} или do end. Нужен чтобы передать и добавить в методы дополнительный код, тоесть инструкцию, а не просто набор значений. Блоки могут быть вложенными, тоесть находиться/передаваться внутри тела других блоков.

[1, 2, 3].each do |el|
  # block - это все что содержится между {} или do end, например тут в итераторе код из блока будет выполняться для каждого элемета массива
...
function_name(par1, par2) do # имя оператора метода/функции к которому будет добавлен этот блок
  # ... код блока
end


# yield - ключевое слово(на самом деле метод), вызывает/исполняет блок, который был передан в метод через синтаксис пристыковывания, код из блока помещается/выполняется в тех местах где стоит yield. yield может передавать значения для параметров блока.
def demo0(n1, n2)
...
# Блок помещенный в переменную при помощи & превращается в процедуру

# call - метод вызывает/исполняет блок кода/процедуру, это действие равнозначное с yield и можно их использовать в одном и том же методе. Может принимать аргументы для блока.
...
def some(par, &block)
  ...
  block.()   #=> 'foo'      # альтернативный способ вызова
  block[]    #=> 'foo'      # альтернативный способ вызова 2
...
# block.call так же как и yield может принимать аргументы для передачи в блок
def some(n, &block)
  block.call(n, 3)
end
...
# Преимущество процедур в том что в отличие от просто блоков метод может принимать множество процедур, тк они передаются как обычные параметры

# Proc — это объект, который инкапсулирует блок кода. Вы можете создать объект Proc и вызвать его с помощью метода call.
...
# Lambda — это другой тип объекта, который также инкапсулирует блок кода, но ведет себя чуть иначе, чем Proc. Например, он проверяет количество аргументов.
...
# 1. Если при вызове процедуры не передать аргументы, то в их параметры назначатся значения nil. А если передать больше аргументов, чем есть параметров - то они проигнорируются
p = proc { |arg1| arg1 }
p p.call          #=> nil
p p.call(1, 2, 3) #=> 1
