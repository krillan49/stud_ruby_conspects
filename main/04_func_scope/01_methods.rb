...
# Объекты можно передавать между файлами. Поэтому если объявить метод в одном файле, он автоматом цепляется к глобальному объекту main, то в другом файле (при помощи require) его тоже можно вызовать потому, что там тоже есть этот глобальный объект main.
...
# основные правила для имен методов в Ruby:
# 1. Имена методов могут содержать буквы и цифры. Однако имя метода не может начинаться с цифры. Ruby является чувствительным к регистру, поэтому `myMethod`, `MyMethod` и `mymethod` будут считаться разными методами.
# 2. Знаки подчеркивания (`_`) для разделения слов в имени метода.
# 3. Специальные символы, такие как `?`, `!`, и `=`. Например:
# `is_valid?` — принято если, что метод возвращает булево значение.
# `set_value!` — принято указывать на то, что метод может изменять состояние объекта.
# `update_status=` — следует за методом установки значения.
# 4. Символы  `@` и `$` для специальных методов
...
# return - оператор возвращает из функции значение указаннй после него строки кода к оператору вызова метода и завершает работу метода. По умолчанию любой метод возвращает значение последней строки кода, тоесть для его возврата прописывать return не обязательно
