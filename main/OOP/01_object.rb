...
# Любой кусок(блок) кода исполняется интерпритатором, в контексте какого-то объекта, даже в корневой области видимости (объект main класса Object)
p self       #=> main
p self.class #=> Object
...
# object_id показывает адрес в памяти определённого объекта. Он нужен для того, чтобы сравнивать разные объекты между собою, потому что порой объекты с одинаковым значением имеют разный object_id, а иногда наоборот
