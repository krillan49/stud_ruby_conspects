...
# Наследование(inheritance) - позволяет создать класс наследник, что получит все методы, поля и конструкторы материнского класса (суперкласса). При этом в каждом классе-наследнике можно добавить свои уникальные методы и свойства, не распространяющиеся на материнский класс или другие классы наследники.
...
# super - ключевое слово используется чтобы вызывать методы и конструкторы суперкласса в одноименных методах и конструкторах подкласса, если нужно расширить или изменить наследуемое поведение этих методов или конструкторов. По сути это просто в методе подкласса вызов одноименного метода суперкласса через его оператор, с возможностью передать ему параметры, если они предполагаются.
