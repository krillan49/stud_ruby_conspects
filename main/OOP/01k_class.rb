...
# Класс - это тип данных, описывающий каждый экземпляр этих данных, тоесть класс это описание объектов,которые будут использоваться в программе

# У каждого класса есть набор методов, которые определяют, что объекты этого класса могут длать - это называется интерфейс класса

# Есть встроенные клаасы в Ruby: Integer, String, Array итд; либо свои классы.
...
# Класс есть смысл создавать только если необходим объект этого класса, а иначе это будет просто набор методов

# Классы принято создавать каждый в отдельном фаиле с тем же названием что и класс, только в снэйккейс.

# Состояние объекта (state) - это совокупность значений всех его переменных (свойств) в данный момент (есть в cs)



puts '                                     Синтаксис, экземпляр. Метод new'

# Методы экзмпляра инкапсулированы в соответсвующий класс и могут быть вызвани только от экземпляра
# Классы называют существительными, как правило в единственном числе, в камел кейс и каждое слово с большой буквы. А фаил в котором находится класс принято называть так же как класс, но в снэйк кейс
class MyAnimal
...
puts '                                 Переменные экземпляра (атрибуты / поля)'
...
# Переменная экземпляра / instance variable - переменная экземпляра класса будет видна только внутри тела этого класса и доступна в каждом методе экземпляра класса. Каждый объект имеет свою собственную копию этих переменных, и их значения могут отличаться для разных объектов

# “@” (читается как “at”) - с этог символа начинаются все переменные экземпляра

# Переменные образца класса являются атрибутами или полями класса, тк сохраняют свое состояние для образца класса. Данные атрибутов сохраняются отдельно в областях памяти приписанных этим объектам, тоесть это данные объектов
