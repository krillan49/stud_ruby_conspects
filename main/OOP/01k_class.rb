...
class Car
  ...
  attr_accessor :color, :name, :name2   # либо можно задавать с новой строки с нового кодового слова
  attr_reader :wheels
  ...
  def initialize(name)
    ...
    # Альтернативный метод инициальзации переменной экземпляра при помощи сеттера тк self.name2 = это вызов метода экзэмпляра name2= в котором есть соответсвующая переменная @name2
    self.name2 = name
  end
end
