...
# Клаасы в Руби это тоже объекты и у них могут быть свои переменные, как у любого объекта
...
puts
# Можно обращаться к методу экземпляра класса в теле класса как обычному методу, например в другом методе класса, но он вызовется тоже от объекта, тк применит к нему self по умолчанию
class Animal
  def run
    puts "I am runing"
  end
  ...
  def eat
    print 'I am eating '
    run # вызываем метод экземпляра run в теле метода eat, теперь метод при вызове от объекта вызовет и метод run
  end
end
cat = Animal.new
...
# Методы Setter возвращают не то, что вы могли бы подумать. Когда вы используете синтаксический сахар, который позволяет вам делать вызовы методов = похожими на присваивания, Ruby серьезно относится к семантике присваивания. Присваивания (например, x = 1) вычисляются с учетом того, что находится справа от них. Методы обычно возвращают значение последнего выражения, вычисленного во время выполнения. Но вызовы метода = ведут себя как присваивания: значение выражения ticket.price = 63.00 равно 63.00, даже если метод ticket= возвращает строку “Ха-ха!”. Идея состоит в том, чтобы сохранить согласованность семантики. По сути, это вызов метода; но он выглядит как присваивание и ведет себя как присваивание в отношении своего значения в виде выражения.
class Some
  def initialize
    @n = 1
  end
...
# Есть 3 метода для создания свойств класса: attr_reader, attr_writer и attr_accessor.
# Когда мы обявляем attr_ конструкцию, то автоматически создаются метод(ы) для каждой переменной(name= для writer; name для reader; name= и name для accessor) и соответсвующая неинициализированная переменная
# attr_accessor - (RW) переменные задаются и для режима чтения и для режима записи/изменения значения переменных (Содержит в себе и accessor и setter methods)
...
bmw = Cart.new
bmw.speed = 230        # присваиваем значениt(инициализируем) переменной экземпляра @speed, заданным классом(тк метод= существует "внутри" attr_ конструкции)
...
bmw.speed      #=> 230             # Вывод значения переменной экземпляра через обращение к атрибуту
