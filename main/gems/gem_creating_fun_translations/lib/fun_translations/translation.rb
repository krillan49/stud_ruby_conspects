module FunTranslations
  class Translation
    ...
    def initialize(raw_translation) # raw_translation - хэш вида: {"translated": "Force be with you my padawan!", "text": "Hello my padawan!",  "translation": "yoda" } принятый в методе translate класа Client
      if raw_translation['translated'].respond_to?(:key?) && raw_translation['translated'].key?('audio') # проверяем содержит ли ответ аудио(закодированное), тк тогда в разделе 'translated' будет не текст перевода, а ключ 'audio', так же дополнительно в основном жеше есть ключи настроек аудио('speed', 'tone')
      ...
      @original_text = raw_translation['text']           # тот текст что мы посылали для перевода
      @translation = raw_translation['translation']      # тип перевода, например пират или Йода
    end
  end
end
