...
# 1. Создадим спецификацию (gemspec) для нашего гема в корне проекта. Описывает что это за библиотека. Нужна для того чтобы библиотека была потом правильно скомпилирована и правильно установлена (сбилдена)
...
# https://github.com/bodrovis/fun_translations   - код готового гема, с докумментацией из комментов в фаилах

# /CHANGELOG.md  - стоит добавлять этот фаил, чтобы пользователи понимали, что изменилось в каждой новой версии

# /LICENSE.md - фаил с описанием лицензии нашего гема

# /Rakefile - необязательный фаил с дополнительными задачами для нашей библиотеки. (Закомментил  его на всякий)

# /README.md  - фаил с описанием/докумментацией библиотеки

# /.github/ - директория со спец фаилами для Гитхаб
# /.github/CODE_OF_CONDUCT.md   - описывает наши стандарты, что не поддерживаем дискриминацию итд чухня
# /.github/CONTRIBUTING.md  - описывает какие действия нужно выполнить, чтоб участвовать в разработке этого проекта
# /.github/dependabot.yml  - говорит гитхабу чтобы он проверял зависимости этой библиотеки и сообщал если есть чтото новое (Заккоментил на всякий)
# /.github/PULL_REQUEST_TEMPLATE.md  - используется как шаблон действий, для того кто хочет открыть пул реквест в нашем репозитарии
# /.github/ISSUE_TEMPLATE/  - директория содержит шаблоны для:
# /.github/ISSUE_TEMPLATE/bug_report.md  - шаблон для баг репорта в наш репозитарий
# /.github/ISSUE_TEMPLATE/feature_request.md
# /.github/workflows/  - директория
# /.github/workflows/ci.yml - настроечный фаил, чтобы при каждом пуше в проект запускать автоматически тесты(интеграционные ?) на гитхаби для разных версий Руби(закомментил на всякий)
...
puts '                                           Релиз библиотеки'

# На Гитхабе справа в пункте Releases стоит добавлять каждую последующую версию, чтобы всем было понятно, так же там есть возможность их переключать

# Собственно публикуем програмный код нашей библиотеки на https://rubygems.org, для этого:
# 1. Выполним команду компиляции гема в бинарный фаил, из корня нашего гема:
# > gem build fun_translations.gemspec
# 2. Передаем на https://rubygems.org получившийся бинарный фаил гема при помощи команды:
# > gem push fun_translations-0.0.1.gem

# Все теперь люди могут устанавливать себе этот гем любым способом
