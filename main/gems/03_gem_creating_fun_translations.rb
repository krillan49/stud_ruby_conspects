...
# spec/support/ - создадим директорию для вспомогательных методов и подключим все фаилы из нее в spec_helper.rb
...
# /fun_translations.gemspec - добавим новую зависимость с режимом установки с гемом webmock, чтобы имититовать ответы от API в тестах
# Так же подключим webmock в spec_helper.rb
...
# /spec/lib/fun_translations/client_spec.rb - создадим тесты для клиента и метода translate, тоесть для основного функционала нашего гема, что косвенно протестирует и все зависимые методы
# Так же создадим для него метод имитирующий создание клиента в spec/support/test_client.rb и подключим его модуль в spec_helper.rb

# /spec/lib/fun_translations/error_spec.rb - создадим тесты проверяющие, что при ошибках вызываются правильные исключения



puts '                                               Рубокоп'

# /fun_translations.gemspec - добавим новую зависимость с гемом rubocop и его подгемами
# > bundle u

# /.rubocop.yml - создадим конфигурационный фаил в корне проекта

# Ну и собственно сделаем проверки



puts '                                           Дополнительные фаилы'
