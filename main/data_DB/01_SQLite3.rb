...
db = SQLite3::Database.new('test.sqlite') # подключаем/создаем базу данных с именем test.sqlite как объект класса Database, прописывая к ней путь и присваиваем ее в переменную. Если этот фаил уже создан то откроет его, а если не создан то создаст с этим именем, тут в текущем каталоге приложения(можно поменять путь на любой).
# Расширения для фаила: sqlite, sqlite3, db ...
