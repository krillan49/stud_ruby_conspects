...
# цифры в именах каталогах в Руби преоблазуются в цифры а не строки, например интеджер

# (! потом проверить) Интерполяция внутри yml-фаила (только в самого себя ?)
# имя_каталога1: ${{имя_каталога2.имя_подкаталога}}
# Тоесть вставляем под ключем 'имя_каталога1' значение из 'имя_каталога2.имя_подкаталога'
...
# Так нужно создавать пустые массивы, просто [] в yml будет строкой
