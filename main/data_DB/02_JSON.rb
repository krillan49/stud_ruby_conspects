...
# JSON - (JS Object Notationд) универсальный формат хранения данных(по сути большой хэш или строка отформатированная как хэш ??). Лучше например того формата что мы придумали для локалсторедж в пицашоп, тк там пришлось для него писать парсер, а тут они уже написаны.
