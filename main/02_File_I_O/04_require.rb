...
# require переводится как "требовать/запросить"
...
# require_relative - загружает/подключает фаил с указанным путём относительно местоположения/директории текущего фаила, тоесть того фаила в котором прописано, вне зависимости от того какая текущая директория в терминале в котором он будет исполнен, например если исполняется как "ruby some/test.rb"
...
# Можно использовать __FILE__ в require, получится аналог require_relative:
current_path = "./" + File.dirname(__FILE__)
require "#{current_path}/tools/some_code_2"
...
puts '                                 Подключение множества фаилов одной строкой'

# Подключение в текущий фаил всех фаилов из директории(тут support/) и вложенных директорий по относительному адресу
Dir["#{File.dirname(__FILE__)}/support/**/*.rb"].sort.each {|f| require f}
# File.dirname(__FILE__) - директория текущего фаила
# __FILE__               - константа, указывающая на текущий фаил
