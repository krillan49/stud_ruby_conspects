...
# gets все символы введенные с клавиатуры, а нажатие Enter генерирует символ перевода строки, соответственно в переменную присваивается не только введеный текст но и спецсимвол перевода строки "\n".
# Для того, чтоб переход на новую строку не сохранялся можно воспользоваться строковыми методами chomp или strip
name = gets.chomp # chomp удалит только 1 последний пробельный элемент
...
# Изменить кодировку принятой строки, тк малоли какой-то ебнутый терминал (например старой Винды) передает не в UTF-8
...
# STDIN  - (STandarD INput) это объект, который содержит в себе ссылку на консоль во время выполнения программы


# Просто gets - это метод Kernel#gets. Если при запуске программы был передан аргумент (который позже можно получить в программе из массива ARGV), то он воспримет его как путь к файлу и попытается этот файл прочитать, а у пользователя в консоли ничего не просит:
# > ruby main.rb Привет
...
# STDIN.gets - метод объекта стандартного ввода, оболее строгий чем Kernel#gets и будет совершенно точно читать именно из консоли, не отвлекаясь на ARGV.

input = STDIN.gets.chomp


# Правило буравчика: если программа использует ARGV, то вместо gets необходимо использовать STDIN.gets, если не использует — можно использовать и Kernel#gets и STDIN.gets
