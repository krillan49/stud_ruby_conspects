...
# File - встроенный в Руи класc отвечает за работу с фаилами.
# Dir - класс отвечает за работу с директориями

# ! обрабатываются в путях только такие "/" слэши


# Стоит читать все данные из фаила до того как запускать логику программы, тк например при работе с пользователей в этом случае испорченный фаил(например некорректые теги в XML) сразу можно будет исключить, а не сообщять пользователю что все сломалось, когда он уэе сделал кучу действий
...
# (? Устарело) Так же можно указывать режим с кодировкой, например:
'r:UTF-8'
...
# __FILE__ - зарезервированная константа, которая возвращает путь к тому фаилу, который сейчас выполняется, тоесть путь к фаилу из которого она вызвана

# Обычный вариант:
file_path = "./text/simple.txt"
File.new(file_path, "r")
# Тут "." в пути обозначает не ту папку где лежит фаил с этим скриптом, а текущую папку в терминале, тоесть ту откуда мы запускаем интерпритатор ruby, а запустить мы его можм и так "ruby test.rb" тоесть прямо из текущей директории, так и по пути находясь в другой текущей директории, например "ruby some/test.rb", соответсвенно при таком маршруте фаил уже не будет найден

# Универсальный вариант с __FILE__:
current_path = File.dirname(__FILE__)
file_path = "#{current_path}/text/simple.txt"
File.new(file_path, "r")
# Так при вызове интерпритатора из другой директории, например "ruby some/test.rb", все бует работать

# Можно использовать так же и в require, получится анаог require_relative:
current_path = "./" + File.dirname(__FILE__)
require "#{current_path}/text/test.rb"



puts '                                 File.new (Создание фаила и методы чтения)'

# File.new - метод для открытия/создания фаила (объект File), не может принимать блок. Подходит для чтения, записи или того и другого одновременно, в зависимости от строки режима. Если такого фаила нет, то выдаст ошибку.

# Присваиваем объект фаила в переменную:
file = File.new("./text/simple.txt", "r") #=> #<File:./text/simple.txt>

# read - метод вернет все содержимое фаила в виде строки
str = file.read #=> "вася вася возвращаемся\nпетя пертя помещается\nколя коля коля\n"

# Второе применение чтения фаила возвращает пустую строку, тк он уже прочитан
file.read #=> ""
...
# readlines - метод возвращает массив, элементы которого это строки фаила. Так же как и предыдущие читает только недочитанную часть фаила, те с той строки или символа на котором отсановилось чтение. Не вызывает ошибку для уже прочитанного фаила
...
# close - метод чтобы закрыть этот файл. Так то Руби после закрытия программы автоматически закрывает все фаилы, но каждый открытый фаил это потраченные ресурсы ОС, которые будут занячты этим открытым фаилом, пока программа на Руби работает. Так же возможны конфликты если тот же фаил понадобится лругой программе или другой части нашей же программы
file.close
...
# File.read - сразу читает фаил (?? в режиме r ??). Метод сам открывает фаил считывает данные и закрывыает фаил
...
output = File.open("./text/simple.txt", "w") # открываем в режиме w
# Так же можно использовать и File.new
...
output.write("\nSomething", 2) # 2й аргумент это число отступов слева в виде пробелов (удобно для XML)

# (?? устарело потом проверить??) print - метод, записывает в фаил строку
output.print "\nSomething" # можно пользоваться \n, чтоб записывалось с новой строки

# (?? устарело потом проверить??) puts - метод, записывает в фаил строку и добавляет в конце "\n"
...
output.close # без закрытия не сохранит изменения
...
# File.exist? проверяет, существует ли файл, например перед открытием (новые версии Руби с 3.какой-то)
File.exist?('test3.rb') #=> true
File.exist?('test5.rb') #=> false

# File::exists? проверяет, существует ли файл перед его открытием (старые версии Руби до 3.какой-то)
File::exists?("file.rb")
