...
# YAML  - (yet another markup language) формат хранения данных, вида ключ-значение. Проще чем JSON. Используется часто для хранения переводы, конфигурацию и другие не очень сложные и объемные данные.
...
# safe_load_file - метод безопасного(не пускает хак-код) чтения, загружает YAML фаил, обрабатывает и преобразует в обычный хэш или массив хэшей если ключи верхнего уровня - элементы массива
data = YAML.safe_load_file('41_yaml.yml', symbolize_names: true)
# 41_yaml.yml - имя(?относительный маршрут?) соотв YAML фаила
