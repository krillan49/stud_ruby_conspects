...
# logger.rb(разделим функционал и выполнение по разным фаилам):
class Logger
  def initialize
    @f = File.open 'log.txt', 'a' # тоесть фаил будет создаваться/открываться при создании экземпляра(объекта)
  end

  @@x = Logger.new # инициализируем экземпляр класса в теле класса(тк в методе класса не пропустит private_class_method)
